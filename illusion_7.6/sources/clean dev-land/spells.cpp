...
int SpellScript::registerFunctions(){
	...
	lua_register(luaState, "DoRemoveCond", SpellScript::luaRemoveCondition);
...
int SpellScript::djinSpell(lua_State *L){
	bool attacked = (int)lua_tonumber(L, -8);
    unsigned long id = (unsigned long)lua_tonumber(L, -7);
	
    int outfit = (int)lua_tonumber(L, -6);
	
    int head = (int)lua_tonumber(L, -5);
	int body = (int)lua_tonumber(L, -4);
	int legs = (int)lua_tonumber(L, -3);
	int feet = (int)lua_tonumber(L, -2);
	...
	if(attacked){
       ...
       toChange = attackedOne;         
    }
    else{
       if(!creature)//Security measure. Doesnt will crash anyway.
          return 0;     
       
       toChange = creature;
    }
    if(!toChange || toChange == NULL)
       return 0;
                      
	toChange->looktype = outfit;
	
	toChange->lookhead_master   = toChange->lookhead;
	toChange->lookbody_master   = toChange->lookbody;
	toChange->looklegs_master   = toChange->looklegs;
	toChange->lookfeet_master   = toChange->lookfeet;
	   
    toChange->lookhead = head;
	toChange->lookbody = body;
	toChange->looklegs = legs;
	toChange->lookfeet = feet;

    spell->game->creatureChangeOutfit(toChange);
	
    spell->game->changeOutfitAfter(toChange->getID(), toChange->lookmaster, time);
    
    SpectatorVec list;
	SpectatorVec::iterator itc;

	spell->game->getSpectators(Range(toChange->pos, true), list);

	for(itc = list.begin(); itc != list.end(); ++itc) {
	   Player *p = dynamic_cast<Player*>(*itc);
	   if(p){
          p->sendMagicEffect(toChange->pos, NM_ME_MAGIC_ENERGIE);
       }
	}
	return 0;
}
...
int SpellScript::luaRemoveCondition(lua_State* L)
{
    ...
    switch(condition)
    {
        case 0: attackType = ATTACK_NONE; 
            break;
       case 1: attackType = ATTACK_ENERGY; 
            break;

        case 2: attackType = ATTACK_BURST; 
            break;
 
        case 3: attackType = ATTACK_FIRE; 
            break;

        case 4: attackType = ATTACK_PHYSICAL; 
            break;

        case 5: attackType = ATTACK_POISON; 
            break;

        case 6: attackType = ATTACK_PARALYZE; 
            break;
 
        case 7: attackType = ATTACK_DRUNKNESS; 
            break;
    ...
    if (player && creature)
    {
        creature->removeCondition(attackType);
