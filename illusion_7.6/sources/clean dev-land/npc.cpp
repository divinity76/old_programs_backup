...
void Npc::doLook(const Creature *creature)
{
    ...
    int deltax = creature->pos.x - this->pos.x;
    int deltay = creature->pos.y - this->pos.y;
...
int NpcScript::registerFunctions()
{
...
#ifdef NPC_TURNING		
        lua_register(luaState, "selfLook", NpcScript::luaActionLook);
#endif //NPC_TURNING
	...
	lua_register(luaState, "isPremmium", NpcScript::luaIsPremium);
...
int NpcScript::luaIsPremium(lua_State* L)
{
	...
	if(player)
		lua_pushboolean(L, player->premmium);
	else
		lua_pushboolean(L, false);
...
#ifdef NPC_TURNING
int NpcScript::luaActionLook(lua_State* L){
    int id = (int)lua_tonumber(L, -1);
    lua_pop(L,1);
    Npc* mynpc=getNpc(L);
    Creature* c = mynpc->game->getCreatureByID(id);
