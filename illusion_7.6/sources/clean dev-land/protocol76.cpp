...
void Protocol76::parseRequestOutfit(NetworkMessage &msg)
{
	...
	case PLAYERSEX_FEMALE:
		...
		msg.AddByte(player->premmium? PLAYER_FEMALE_7 : PLAYER_FEMALE_4);
#else
...
#ifdef YUR_PREMIUM_PROMOTION
		msg.AddByte(player->premmium? PLAYER_MALE_7 : PLAYER_MALE_4);
#else
...
#ifdef YUR_PREMIUM_PROMOTION
		temp <= (player->premmium? PLAYER_FEMALE_7 : PLAYER_FEMALE_4)) ||
#else
...
#ifdef YUR_PREMIUM_PROMOTION
		temp <= (player->premmium? PLAYER_MALE_7 : PLAYER_MALE_4)))
#else
		temp <= PLAYER_MALE_7))
#endif //YUR_PREMIUM_PROMOTION
	{
...
depotid = 3;
else if(line[1] == g_config.getGlobalString("city4","city4")) // add as much as you want
depotid = 4;
else if(line[1] == g_config.getGlobalString("city5","city5")) // add as much as you want
depotid = 5;
else
canSend = false;
...
unsigned short maxlen= msg.GetU16();//??
...
myIP2 << (unsigned int)ip[0] << "." << (unsigned int)ip[1] <<
     "." << (unsigned int)ip[2] << "." << (unsigned int)ip[3];
myIP = myIP2.str();
     if(player->access > bannedPlayer->access ){
         if(comment.size() > 0 && comment.size() < 9999){
            ...
            switch(action){
             ...
             case 4: Action = "AccountBan"; break;
             case 5: Action = "AccountBan"; break;
