...
#ifdef NAXE_CANTWALK
if(state != STATE_IDLE && !(state == STATE_IDLESUMMON && hasLostMaster) && this->getName() != "Spit Nettle" && this->getName() != "Magicthrower" && this->getName() != "Plaguethrower") {
#else
	//process movement
...
void Monster::reThink(bool updateOnlyState /* = true*/)
{
	if(isSummon()) {
		...
		if(state == STATE_FLEEING) {
		   ...
		   #ifdef _PARALYZE_
       if(mType->canParalyze)
        {
        unsigned short probability = random_range(1,100);
        if(probability <= 5)
        {

            Player* player = game->getPlayerByID(this->attackedCreature);
            if(player && game->map->canThrowObjectTo(this->pos, player->pos, BLOCK_PROJECTILE) == RET_NOERROR && player->paralyzeTicks <= 0 && getCurrentDistanceToTarget(player->pos) <= 7)
            {
              player->hasteTicks = 0;
              //game->changeSpeed(player->getID(), (int)(player->getNormalSpeed()));
              player->paralyzeTicks = 60000;// 1 min
              game->spectatorEffect(player->pos, NM_ME_MAGIC_BLOOD);
              game->changeSpeed(player->getID(), (int)(player->getNormalSpeed()/2));
              player->sendIcons();
			...
			if(hasmelee && paIt->first->fighttype == FIGHT_POISON_MELEE){
				...
				if((attackedCreature->getImmunities()) != ATTACK_POISON){
                    ...
                    if(probability > 45){
                        ...
                        if(player && !isSummon()){
                            if(curPhysicalAttack->maxHitsDamage >= player->cCD){
                                player->sendTextMessage(MSG_EVENT, "You are poisoned.");                                   
        	    		    	game->CreateCondition(player, this, COLLOR_GREEN, NM_ME_POISEN_RINGS, NM_ME_POISEN_RINGS, ATTACK_POISON, true, curPhysicalAttack->maxHitsDamage, curPhysicalAttack->maxHitsDamage, (long)g_config.getGlobalNumber("exhausted", 1500), curPhysicalAttack->maxHits);
        	    		    	player->cCD = curPhysicalAttack->maxHitsDamage;
                            }
                        }
                        else
      	    		    	game->CreateCondition(attackedCreature, this, COLLOR_GREEN, NM_ME_POISEN_RINGS, NM_ME_POISEN_RINGS, ATTACK_POISON, true, curPhysicalAttack->maxHitsDamage, curPhysicalAttack->maxHitsDamage, (long)g_config.getGlobalNumber("exhausted", 1500), curPhysicalAttack->maxHits);
                    }
                    Player *player = dynamic_cast<Player*>(attackedCreature);
                    if(player)
                        player->sendIcons();    	    			
				...
				if((attackedCreature->getImmunities()) != ATTACK_FIRE){
                    ...
                    if(probability > 45){
    	    			...
    	    			game->CreateCondition(attackedCreature, this, COLLOR_BURN, NM_ME_HITBY_FIRE, NM_ME_HITBY_FIRE, ATTACK_FIRE, true, curPhysicalAttack->maxHitsDamage, curPhysicalAttack->maxHitsDamage, (long)g_config.getGlobalNumber("exhausted", 1500), curPhysicalAttack->maxHits);
                    }
                    Player *player = dynamic_cast<Player*>(attackedCreature);
                    if(player)
                        player->sendIcons();    	    			
				...
				if((attackedCreature->getImmunities()) != ATTACK_ENERGY){
                    ...
                    if(probability > 45){
    	    			...
    	    			game->CreateCondition(attackedCreature, this, COLLOR_LIGHT_BLUE, NM_ME_ENERGY_DAMAGE, NM_ME_ENERGY_DAMAGE, ATTACK_ENERGY, true, curPhysicalAttack->maxHitsDamage, curPhysicalAttack->maxHitsDamage, (long)g_config.getGlobalNumber("exhausted", 1500), curPhysicalAttack->maxHits);
                    }
                    Player *player = dynamic_cast<Player*>(attackedCreature);
                    if(player)
                        player->sendIcons();    	    			
