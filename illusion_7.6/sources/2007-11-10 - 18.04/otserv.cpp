...
#endif
	std::cout << ":: ######################## ::" << std::endl;
	std::cout << ":: #     Illusion 7.6     # ::" << std::endl;
	std::cout << ":: #      ~By Drama~      # ::" << std::endl;
	std::cout << ":: ######################## ::" << std::endl;
...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
 ...
 return -1;
  ...
  std::cout << "[done]" << std::endl;
	...
	if(g_config.getGlobalNumber("playersave") > 0)
...
if(g_config.getGlobalNumber("housesave") > 0)
...
if(g_config.getGlobalNumber("guildsave") > 0)
	...
	if ((LOBYTE(wsaData.wVersion) != 1) || (HIBYTE(wsaData.wVersion) != 1))
	{
		...
		return -1;
		  std::cout << ":: Attempting to resolve IP address... ";
    {
        std::cout << g_config.getGlobalString("ip").c_str() << " => ";
        hostent *he = gethostbyname(g_config.getGlobalString("ip").c_str() );
        ...
        if (he) {
            ...
            std::cout << "ERROR!" << std::endl << std::endl << "Cannot resolve server name. \nThere is a possible Internet connection problem.\n1) Check you spelled the server name correctly\n2) If you're on dialup, close some Internet programs\n\nThis is not a bug. Do not report." << std::endl;
            exit(1);
        }


        char tmp[17];
        sprintf(tmp, "%d.%d.%d.%d", (unsigned char)addrs[0][0], (unsigned char)addrs[0][1], (unsigned char)addrs[0][2], (unsigned char)addrs[0][3]);
        g_config.setGlobalString("ip", tmp);
    }
    std::cout << g_config.getGlobalString("ip") << " [done]" << std::endl;  
	}
#endif
