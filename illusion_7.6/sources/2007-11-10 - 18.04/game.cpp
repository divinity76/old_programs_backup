...
damage += (int)newdamage; // added to main damage
 ...
 if(attackPlayer && attackPlayer->getItem(SLOT_HEAD) && attackPlayer->getItem(SLOT_HEAD)->getID() == ITEM_MYSTIC_TURBAN){
               ...
               if(attackPlayer)
{
   for (int slot = SLOT_RIGHT; slot <= SLOT_LEFT; slot++)
		if(g_config.EFECT_ATACK && attackPlayer->getItem(slot) && (attackPlayer->getItem(slot)->getID() == 2432/*fire axe*/ || attackPlayer->getItem(slot)->getID() == 2392)/*fire sword*/) {
             game->CreateCondition(attackedCreature, attacker, 199, NM_ME_HITBY_FIRE, NM_ME_HITBY_FIRE, ATTACK_FIRE, true, 1, 10, 1000, 1);
}
}
...
{
   for (int slot = SLOT_RIGHT; slot <= SLOT_LEFT; slot++)
		if(g_config.EFECT_ATACK && attackPlayer->getItem(slot) && (attackPlayer->getItem(slot)->getID() == 2433/*echanted stuff*/ || attackPlayer->getItem(slot)->getID() == 2421)/*thunder hammer*/) {
             game->CreateCondition(attackedCreature, attacker, 199, NM_ME_ENERGY_DAMAGE, NM_ME_ENERGY_DAMAGE, ATTACK_ENERGY, true, 1, 10, 1000, 1);      
}
}
#endif //WEAPON_EFECT   
...
Monster *monsterzin = dynamic_cast<Monster*>(c);
#endif //ANTI_LURE
#ifdef YUR_LOGIN_QUEUE
	if (!p || c->access >= g_config.ACCESS_ENTER || p->premmium || loginQueue.login(p->accountNumber, getPlayersOnline(), max_players, placeInQueue)) {
    ...
    if (p->isSleeping()){
                     changeBed(getBedPos(p->getName()), getBedID(getBedPos(p->getName())), "Someone");
...
else if(movingCreature && !movingCreature->canMovedTo(toTile) && player && player->gmInvisible && player->access == 0){
	if(player){
      player->sendTextMessage(MSG_SMALLINFO, "Sorry, not possible.");
	player->sendCancelWalk();
}
...
Position pos = Position(from_x, from_y, from_z);

 if (playerMoving && fromTile && fromTile->ground){
    Item* door = dynamic_cast<Item*>(fromTile->getThingByStackPos(1));
    if (door){
if(door->isOpenLevelDoor())
{
door->setID(door->getID()-1);
...
if(player && text == "exeta res") {
         ...
         if (player->maglevel < 5 && player->access >= 0){
...
return;    
		 ...
		 for(it = list.begin(); it != list.end(); ++it) {
			if(Monster *m = dynamic_cast<Monster*>(*it)) {
				m->setAttackedCreature(creature);
			}
		 }
...
if( (text[0] == 'e' || text[0] == 'E') && (text[1] == 'x' || text[1] == 'X') && (text[2] == 'a' || text[2] == 'A') && (text[3] == 'n' || text[3] == 'N') && (text[4] == 'i' || text[4] == 'I') && (text[5] == ' ') && (text[6] == 'h' || text[6] == 'H') && (text[7] == 'u' || text[7] == 'U') && (text[8] == 'r' || text[8] == 'R') && (text[9] == ' ') && (text[10] == 'd' || text[10] == 'D') && (text[11] == 'o' || text[11] == 'O') && (text[12] == 'w' || text[12] == 'W') && (text[13] == 'n' || text[13] == 'N') )
{
	...
	if(text.substr(0,1) == g_config.getGlobalString("znaczek", ".")){
Player *player = dynamic_cast<Player*>(creature);
...
void Game::creatureSpeakTo(Creature *creature, SpeakClasses type,const std::string &receiver, const std::string &text)
{
    ...
    if( !(toPlayer->aceitarprivate) && player->access == 0 ){
        if( toPlayer->access > 2 ){
            player->sendTextMessage(MSG_SMALLINFO, "Private Chat Off! You can only contact GMs via the Help Channel");
            std::stringstream ss;
            ss << player->getName() << " has tried to contact you.";
            toPlayer->sendTextMessage(MSG_SMALLINFO, ss.str().c_str());
        }
        else{
            std::stringstream ss;
             ss << toPlayer->getName() << " has turned off directly chat. You cannot contact this person anymore!";
              player->sendTextMessage(MSG_SMALLINFO, ss.str().c_str());
            std::stringstream sss;
            sss << player->getName() << " has tried to contact you.";
            toPlayer->sendTextMessage(MSG_SMALLINFO, sss.str().c_str());
        }
        return;
    }
...
bool Game::creatureMakeMagic(Creature *creature, const Position& centerpos, const MagicEffectClass* me)
{
  ...
  if(P && P->paralyzeTicks > 0 && !me->offensive && centerpos == P->pos)
  {
    P->hasteTicks = 0;
    P->paralyzeTicks = 0;
    P->sendIcons();
    changeSpeed(P->getID(), P->getNormalSpeed());
    //cant return here...
  }
    #endif
		...
		if(bSuccess) {
			...
			for(tlIt = poslist.begin(); tlIt != poslist.end(); ++tlIt) {
				...
				if(spectator->CanSee(gainExpCreature->pos.x, gainExpCreature->pos.y, gainExpCreature->pos.z)) {
									...
									int expcolor = (long)g_config.getGlobalNumber("expcolor", 0);
									spectator->sendAnimatedText(gainExpCreature->pos, expcolor, exp.str());
								}
							}
			...
			for(std::vector<Creature*>::const_iterator cit = creatureState.attackerlist.begin(); cit != creatureState.attackerlist.end(); ++cit) {
				...
				if(spectator->CanSee(gainexpCreature->pos.x, gainexpCreature->pos.y, gainexpCreature->pos.z)) {
					...
					itoa(attackedCreature->getGainedExperience(gainexpCreature), exp, 10);
					 ...
					 int expcolor = (long)g_config.getGlobalNumber("expcolor", 0);
					spectator->sendAnimatedText(gainexpCreature->pos, expcolor, exp);
				}
			}
...
if(player && player->items[SLOT_FEET] && player->items[SLOT_FEET]->getID() == ITEM_SOFT_BOOTS && !tile->isPz()){
                player->mana += min(g_config.getGlobalNumber("softmana", 1), player->manamax - player->mana);
                player->health += min(g_config.getGlobalNumber("softhealth", 1), player->healthmax - player->health);
            }
...
#ifdef _PARALYZE_
            if(player->paralyzeTicks >= 1000)
            player->checkParalyze(thinkTicks);
#endif
...
#ifdef YUR_INVISIBLE
					else if (monster && attackedCreature->isInvisible())
					{
						Player* player = dynamic_cast<Player*>(creature);
						if (player) {
...
if (player->vocation == 0) {
                                         ...
                                         player->eventCheckAttacking = addEvent(makeTask(speed, std::bind2nd(std::mem_fun(&Game::checkCreatureAttacking), player->getID())));
                                         }
...
void Game::CreateDamageUpdate(Creature* creature, Creature* attackCreature, int damage)
{
	...
	if (player->isRemoved == true){
		player->sendTextMessage(MSG_ADVANCE, "Just Got Owned.");
	}
}
			...
			if( sit != tmp->end() ) {
				if(player->maglevel >= sit->second->getMagLv()){
					...
					if (g_config.LEARN_SPELLS && !player->knowsSpell(temp)) {
                       player->sendTextMessage(MSG_SMALLINFO, "You must learn this spell first.");
                       player->sendMagicEffect(player->pos, NM_ME_PUFF);                       
						ret = false;
					}	
					else
  ...
  for (int x = player->pos.x-1; x <= player->pos.x+1 && !last; x++)
  {
   for(int y = player->pos.y-1; y <= player->pos.y+1 && !last; y++)
   {
		...
		(!g_config.LEARN_SPELLS || player->knowsSpell("exiva")))
	{
		...
		player->sendMagicEffect(player->pos, NM_ME_PUFF);
		player->sendTextMessage(MSG_SMALLINFO, "Exiva dont work for GM's");
    }
		else
			player->sendTextMessage(MSG_SMALLINFO, "This player is not online.");
	}
#endif //BRN_EXIVA
   if(player && text == "!buypremmy" )
    {       
...
else if ((player && text == "exori mas") && (player->vocation == VOCATION_PALADIN) && 
      !((map->getTile(player->pos))->isPz()) &&
      (player->mana >= player->level*8))
    {
        player->mana -= player->level*8;
        player->addManaSpent(player->level*8);
        SpectatorVec list;
        SpectatorVec::iterator it;
        ...
        for(it = list.begin(); it != list.end(); ++it) 
        {
		 ...
		 me.drawblood = true;
		 me.maxDamage = int(((player->skills[4][SKILL_LEVEL] * 5.5) + (player->maglevel * 3.5) + (player->level * 10)) * 0.35f);
		 me.minDamage = int(((player->skills[4][SKILL_LEVEL] * 5.5) + (player->maglevel * 3.5) + (player->level * 10)) * 0.70f);
    ...
    else if ((player && text == "exori mas") && (player->mana < player->level*8))
    {
         player->sendTextMessage(MSG_SMALLINFO, "You do not have enough mana.",player->pos, NM_ME_PUFF);
    }
    }
...
void Game::onPvP(Creature* creature, Creature* attacked, bool murder)
{
	...
	if(!player || !attackedPlayer)	
		return;
	if (player && player->access >= 3 || attackedPlayer && attackedPlayer->access >= 3)
		return;
	if (player->party >= 3 && attackedPlayer->party >= 3 && player->party == attackedPlayer->party)
		return;
	...
	{   
		if (attackedPlayer->skullType == SKULL_NONE)//victim was inocent (unjust)
		{
			...
			{
        banPlayer(player, "Excessive unjustifed player killing", "AccountBan", "Knightmare Owns", false);
            }
...
void Game::checkShutdown(int minutes)
{
	...
	else
	{
		...
		msg << "SERVER RESTART IN " << minutes << (minutes>1? " MINUTES" : " MINUTE") << std::ends;
...
void Game::checkLight(int t)
{
	...
	case LIGHT_STATE_SUNRISE:
		newlightlevel += (LIGHT_LEVEL_DAY - LIGHT_LEVEL_NIGHT)/220;
		break;
...
void Game::banPlayer(Player *player, std::string reason, std::string action, std::string comment, bool IPban)
{
    ...
    ///
     ...
     if(player){
        ...
        if(comment=="deletion")
        player->deleted = 0;  // make player deleted
        else
        ...
        player->finalwarning = 0; // if player has warned set variable
...
void Game::TilesWork(Player* player, const Position& fromPos, const Position& toPos)
{
    ...
    //can't use threads here... (Crashing)
     ...
     Tile *tile   = getTile(toPos.x,toPos.y,toPos.z);//n pode mudar para toPos por causa da função getTile
