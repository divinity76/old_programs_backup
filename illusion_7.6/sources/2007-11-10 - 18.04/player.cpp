...
Creature()
		...
		,RED_UNJUST(g_config.getGlobalNumber("redskull",3)),
		BAN_UNJUST(g_config.getGlobalNumber("redskullban",6))
 ...
 walkTo.type = WALK_NONE;
  ...
  //set item pointers to NULL
 	...
 	*/
       ...
       if (premmium)
 	{
	max_depot_items = g_config.MAX_DEPOT_ITEMS_PREMIUM;
#else
...
void Player::dropLoot(Container *corpse)
{
	...
	if (dynamic_cast<Container*>(item) && (player->promoted == 1))
		{
			if (random_range(1, 100) <= g_config.DIE_PERCENT_BP_2)
			{
				corpse->addItem(item);
				items[slot] = NULL;
			}
		}
		else if (random_range(1, 100) <= g_config.DIE_PERCENT_EQ_2)
		{
			corpse->addItem(item);
			items[slot] = NULL;
		}
...
#ifdef _PARALYZE_
void Player::checkParalyze(int thinkTicks)
{
paralyzeTicks -= thinkTicks;
if(paralyzeTicks < 1000)
{
