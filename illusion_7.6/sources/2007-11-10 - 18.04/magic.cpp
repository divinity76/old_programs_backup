...
int MagicEffectClass::getDamage(Creature *target, const Creature *attacker /*= NULL*/) const
{
	if((attackType != ATTACK_NONE) && (target->getImmunities() & attackType) == attackType)
		return 0;
		//Begin protect system - Made by Jero and Junkfood(aka Silvest)
Player *attacked = dynamic_cast<Player*>(target);
bool protection = true;
const Monster *monster = dynamic_cast<const Monster*>(attacker);
if(attacker && attacked && g_game.getWorldType() != WORLD_TYPE_NO_PVP && !monster)
 {
  if(attacker->level >= 1 && attacker->level <= 20 && attacked->level > 20 || attacker->level >= 1 && attacker->level <= 20 && attacked->level < 21)
   protection = false;
  else if(attacker->level >= 21 && attacker->level <= 30 && attacked->level > attacker->level + 20 || attacker->level >= 21 && attacker->level <= 30 && attacked->level < attacker->level - 10 || attacker->level >= 21 && attacker->level <= 30 && attacked->level < 21)
   protection = false;
  else if(attacker->level >= 31 && attacker->level <= 50 && attacked->level > attacker->level + 20 || attacker->level >= 31 && attacker->level <= 50 && attacked->level < attacker->level - 20 || attacker->level >= 31 && attacker->level <= 50 && attacked->level < 21)
   protection = false;
  else if(attacker->level >= 51 && attacker->level <= 60 && attacked->level > attacker->level + 30 || attacker->level >= 51 && attacker->level <= 60 && attacked->level < attacker->level - 20)
   protection = false;
  else if(attacker->level >= 61 && attacker->level <= 70 && attacked->level > attacker->level + 30 || attacker->level >= 61 && attacker->level <= 70 && attacked->level < attacker->level - 20)
   protection = false;
  else if(attacker->level >= 71 && attacker->level <= 80 && attacked->level > attacker->level + 30 || attacker->level >= 71 && attacker->level <= 80 && attacked->level < attacker->level - 30 || attacker->level >= 71 && attacker->level <= 80 && attacked->level < 51)
   protection = false;
  else if(attacker->level >= 81 && attacker->level <= 90 && attacked->level > attacker->level + 30 || attacker->level >= 81 && attacker->level <= 90 && attacked->level < attacker->level - 30)
   protection = false;
  else if(attacker->level >= 91 && attacker->level <= 100 && attacked->level > attacker->level + 30 || attacker->level >= 91 && attacker->level <= 100 && attacked->level < attacker->level - 30)
   protection = false;
  else if(attacker->level >= 101 && attacker->level <= 130 && attacked->level < attacker->level - 30 || attacker->level >= 131 && attacked->level <= 100)
   protection = false;
}
if(attacker && attacked && !protection && !monster && offensive)
{
attacker->sendCancel("1337 protection system!");
return 0;
}
//End protect system - Made by Jero and Junkfood(aka Silvest)
