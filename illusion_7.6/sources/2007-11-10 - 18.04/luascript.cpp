...
int LuaScript::OpenFile(const char *filename)
{
...
#ifdef ADVANCE_ANI
        AD_ANI = getGlobalString("animations") == "yes";
#endif //ADVANCE_ANI
#ifdef WEAPON_EFECT		
		EFECT_ATACK = getGlobalString("atackanimations") == "yes";
#endif //WEAPON_EFECT	
	...
	EXHAUSTED = getGlobalNumber("exhausted",1000);
	EXHAUSTED_ADD = getGlobalNumber("exhaustedadd",300);
	EXHAUSTED_HEAL = getGlobalNumber("exhaustedheal",800);

	PZ_LOCKED = getGlobalNumber("pzlocked",0);
	MAX_DEPOT_ITEMS = getGlobalNumber("maxdepotitem",1000);
	MAX_DEPOT_ITEMS_PREMIUM = getGlobalNumber("maxdepotitempremium",2000);
...
DSM_FIRE = getGlobalNumber("dsmfire",20);
