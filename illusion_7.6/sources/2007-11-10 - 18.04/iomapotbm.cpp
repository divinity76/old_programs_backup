					...
					if(tile){
						...
						while(propStream.GET_UCHAR(attribute)){
							...
							case OTBM_ATTR_ITEM:
								...
								if(item){
                                         if(item->getID() == GROUND_WATER || item->getID() == GROUND_GRASS || item->getID() == GROUND_SWAMP || item->getID() == GROUND_EARTH || item->getID() == GROUND_LAVA || item->getID() == GROUND_ROCK || item->getID() == GROUND_ROCK2)
                                         generateRandom(item);
									item->pos.x = px;
									item->pos.y = py;
									item->pos.z = pz;
...
void IOMapOTBM::generateRandom(Item* item)
{
      int change = 1;
      int newID = 0; 
      switch(item->getID()) //Didnt want the same change rates on grass as for water..
      {
              case GROUND_WATER:
              case GROUND_SWAMP:
                      newID = rand()%15;
                      change = rand()%1; //50% chance to change
              break;
              case GROUND_EARTH:
                      newID = rand()%4;
                      change = rand()%1; //50% chance to change
