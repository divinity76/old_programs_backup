  		...
  		if talk_state == 0 then
 			...
 			if msgcontains(msg, 'found') then	-- found a new guild
  			...
  			elseif msgcontains(msg, 'title') then
  				...
  				if gstat == LEADER then
  					selfSay('Whom player do you want to change title?')
  					talk_state = 11
  				else
  					selfSay('Sorry, only leader can change titles.')
  					talk_state = 0
  				end
  			end
  			...
  			elseif talk_state == 11 then	-- get name of player to change title
 				...
 				gname = getPlayerGuildName(cname)
  				...
  				if gname == gname2 then
  					selfSay('And what title do you want him to have (say none to clear)?')
  					talk_state = 12
  				else
  					selfSay('Sorry, ' .. pname .. ' is not in your guild.')
  					talk_state = 0
  				...
  				if msg == 'none' then
  					...
  					if string.len(msg) <= maxtitlelen then
 						if string.find(msg, allow_pattern) then
  						...
  						selfSay('Sorry, title cannot be longer than ' .. maxtitlelen .. ' characters.')
  						talk_state = 0
  					end
  				end
  			end
