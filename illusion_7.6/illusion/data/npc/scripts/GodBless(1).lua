-- the id of the creature we are attacking, following, etc.
 ...
 cost = 10000
  ...
  function onCreatureSay(cid, type, msg)
  	...
  	if ((string.find(msg, '(%a*)bless(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 then
  		selfSay('Hmm... So you want be blessed by Gods.... Are you shure about That?')
 		talk_start = 1
 	end
 
  	if ((string.find(msg, '(%a*)yes(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 and talk_start == 1 and creatureGetBlessStatusb(cid) == 0 then
 ...
 if pay(cid,cost) then
  		...
  		selfSay('All the Gods accepted you to be blessed... So be it!')
 	else
  	...
  	if ((string.find(msg, '(%a*)yes(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 and talk_start == 1 and creatureGetBlessStatusb(cid) >= 1 then
  		selfSay('You already got Your bless...')
 		talk_start = 0
 	end
 
  	if ((string.find(msg, '(%a*)no(%a*)')) and (focus == cid)) and getDistanceToCreature(cid) < 4 and talk_start == 1 and creatureGetBlessStatusb(cid) >= 0 then
  	...
  	if string.find(msg, '(%a*)bye(%a*)') and focus == cid and getDistanceToCreature(cid) < 4 and creatureGetBlessStatusb(cid) >= 1 then
  	...
  	if string.find(msg, '(%a*)bye(%a*)') and focus == cid and getDistanceToCreature(cid) < 4 and creatureGetBlessStatusb(cid) == 0 then
