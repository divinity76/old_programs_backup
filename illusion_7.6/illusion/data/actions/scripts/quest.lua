   ...
   function onUse(cid, item, frompos, item2, topos)
   	...
   	elseif item.actionid == 5001 then
   		...
   		if queststatus == 4 then
   			...
   			doPlayerSendCancel(cid,"Only some special people can use is empty.")
    end
   	elseif item.uid == 1378 then
   		queststatus = getPlayerStorageValue(cid,1378)
   		if queststatus == -1 then
   			doPlayerSendTextMessage(cid,22,"You have found Plasma shield.")
   			doPlayerAddItem(cid,2542,1)
   			setPlayerStorageValue(cid,1378,1)
   		else
   			doPlayerSendTextMessage(cid,22,"It is empty.")
	end
