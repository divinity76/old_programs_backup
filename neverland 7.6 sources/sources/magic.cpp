...
if(offensive){
attacker->sendCancel("Gms cant attack players");
return 0;
}
}
		...
		if(spectator->CanSee(pos.x, pos.y, pos.z)) {
			if(damageEffect != 0xFF) {
				if(!offensive || !(g_game.getWorldType() == WORLD_TYPE_NO_PVP && dynamic_cast<const Player*>(attacker) &&
					dynamic_cast<const Player*>(target) && target->access < g_config.ACCESS_PROTECT && attacker->access < g_config.ACCESS_PROTECT) || target->access >= g_config.ACCESS_PROTECT) {
