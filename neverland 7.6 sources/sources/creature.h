...
public:
	...
	virtual int getImmunities() const
	{
		if(access >= g_config.ACCESS_PROTECT) 
			...
			ATTACK_PHYSICAL | ATTACK_POISON | ATTACK_PARALYZE | ATTACK_DRUNKNESS | ATTACK_INVIS;
		else
			return immunities;
	};
#ifdef YUR_PVP_ARENA
...
unsigned long eventCheckFollow;
	...
	long manaShieldTicks, hasteTicks, paralyzeTicks, stealthTicks; 
	#ifdef RUL_DRUNK
...
#ifdef TIJN_UTANAVID
		unsigned int invisTicks;
		unsigned short getItemId() const { return itemid; }
	void setItemId(unsigned short newid) { itemid = newid; }
bool isInvisible() const { return stealthTicks > 0; }
	void setInvisible(int ticks) { stealthTicks = ticks; }
#endif //TIJN_UTANAVID

unsigned int chameleonTicks;
...
protected:
	...
	// now in public Conditions conditions;
