...
NpcScript::NpcScript(std::string scriptname, Npc* npc){
	...
	this->loaded=true;
	this->npc=npc;
	this->setGlobalNumber("addressOfNpc", reinterpret_cast<int>(npc));
...
{
    lua_register(luaState, "Walk", NpcScript::luaSelfWalk);
...
lua_register(luaState, "isMarried", NpcScript::luaisMarried);
	...
	lua_register(luaState, "selfGetPosition", NpcScript::luaSelfGetPos);
	lua_register(luaState, "getPlayerHealth", NpcScript::luaGetPlayerHealth);
lua_register(luaState, "healPlayer", NpcScript::luaHealPlayer);
#ifdef RUL_BLESS
lua_register(luaState, "creatureGetBless", NpcScript::luaCreatureGetBless);
lua_register(luaState, "setNewBless", NpcScript::luaSetNewBless);
lua_register(luaState, "creatureGetBlessStatusa", NpcScript::luaCreatureGetBlessStatusa);
lua_register(luaState, "creatureGetBlessStatusb", NpcScript::luaCreatureGetBlessStatusb);
lua_register(luaState, "creatureGetBlessStatusc", NpcScript::luaCreatureGetBlessStatusc);
lua_register(luaState, "creatureGetBlessStatusd", NpcScript::luaCreatureGetBlessStatusd);
lua_register(luaState, "creatureGetBlessStatuse", NpcScript::luaCreatureGetBlessStatuse);
#endif //RUL_BLESS
#ifdef TIJN_SHIP
lua_register(luaState, "Travel", NpcScript::luaTravel);
#endif //TIJN_SHIP
lua_register(luaState, "getPlayerItem", NpcScript::luaGetPlayerItem);
...
lua_register(luaState, "BuyContainer", NpcScript::luaBuyCont);
	...
	lua_register(luaState, "isPremium", NpcScript::luaIsPremium);
	lua_register(luaState, "promote", NpcScript::luaPromote);
...
Npc* NpcScript::getNpc(lua_State *L){
	...
	Npc* mynpc = reinterpret_cast<Npc*>(val);
	if(!mynpc){
		return 0;
	}
	return mynpc;
...
int NpcScript::luaIsPremium(lua_State* L)
{
	...
	if (player)
		lua_pushboolean(L, player->premium);
	else
		lua_pushboolean(L, false);
...
#ifdef RUL_BLESS
/* Bless System - By Kyo */
int NpcScript::luaCreatureGetBless(lua_State *L){
int id = (int)lua_tonumber(L, -1);
lua_pop(L,1);
Npc* mynpc = getNpc(L);
Creature *target = mynpc->game->getCreatureByID(id);
lua_pushnumber(L, target->bless);
return 1;
}
int NpcScript::luaSetNewBless(lua_State *L){
int bless = (int)lua_tonumber(L, -1);
int blessa = (int)lua_tonumber(L, -2);
int blessb = (int)lua_tonumber(L, -3);
int blessc = (int)lua_tonumber(L, -4);
int blessd = (int)lua_tonumber(L, -5);
int blesse = (int)lua_tonumber(L, -6);
int cost = (int)lua_tonumber(L, -7);
int id = (int)lua_tonumber(L, -8); 
lua_pop(L,9);
Npc* mynpc = getNpc(L);
Creature *target = mynpc->game->getCreatureByID(id);
Player *player = mynpc->game->getPlayerByID(id);
if (player && player->removeCoins(cost)){ // if player have gold ;>
target->bless = bless;
target->blessa = blessa;
target->blessb = blessb;
target->blessc = blessc;
target->blessd = blessd;
target->blesse = blesse;
}
return 1;
}
/* Checando o Status das Blesses */
int NpcScript::luaCreatureGetBlessStatusa(lua_State *L){
int id = (int)lua_tonumber(L, -1);
lua_pop(L,1);
Npc* mynpc = getNpc(L);
Creature *target = mynpc->game->getCreatureByID(id);
lua_pushnumber(L, target->blessa);
return 1;
}
int NpcScript::luaCreatureGetBlessStatusb(lua_State *L){
int id = (int)lua_tonumber(L, -1);
lua_pop(L,1);
Npc* mynpc = getNpc(L);
Creature *target = mynpc->game->getCreatureByID(id);
lua_pushnumber(L, target->blessb);
return 1;
}
int NpcScript::luaCreatureGetBlessStatusc(lua_State *L){
int id = (int)lua_tonumber(L, -1);
lua_pop(L,1);
Npc* mynpc = getNpc(L);
Creature *target = mynpc->game->getCreatureByID(id);
lua_pushnumber(L, target->blessc);
return 1;
}
int NpcScript::luaCreatureGetBlessStatusd(lua_State *L){
int id = (int)lua_tonumber(L, -1);
lua_pop(L,1);
Npc* mynpc = getNpc(L);
Creature *target = mynpc->game->getCreatureByID(id);
lua_pushnumber(L, target->blessd);
return 1;
}
int NpcScript::luaCreatureGetBlessStatuse(lua_State *L){
int id = (int)lua_tonumber(L, -1);
lua_pop(L,1);
Npc* mynpc = getNpc(L);
Creature *target = mynpc->game->getCreatureByID(id);
lua_pushnumber(L, target->blesse);
return 1;
}
/* Checando o Status das Blesses */
/* Bless System - By Kyo */
...
int NpcScript::luaGetPlayerHealth(lua_State* L)
{
    int id = (int)lua_tonumber(L, -1);
    lua_pop(L, 1);
    ...
    if (player)
        lua_pushnumber(L, player->getPlayerInfo(PLAYERINFO_MAXHEALTH));
    else
        lua_pushnumber(L, -1);
...
int NpcScript::luaHealPlayer(lua_State* L)
{
	int id = (int)lua_tonumber(L, -4);
	int health = (int)lua_tonumber(L, -3);
	lua_pop(L, 4);
	...
	if (player){
          player->health += std::min(health, player->healthmax - player->health);
          player->sendStats();
          SpectatorVec list;
          SpectatorVec::iterator it;
          mynpc->game->getSpectators(Range(player->pos, true), list);
	     for(it = list.begin(); it != list.end(); ++it){
               Player* spectators = dynamic_cast<Player*>(*it);
               if(spectators){
                    spectators->sendMagicEffect(player->pos, NM_ME_MAGIC_ENERGIE);
                    spectators->sendCreatureHealth(player);
               }
          }
     }
	else
...
int NpcScript::luaSelfWalk(lua_State* L)
{   
 ...
 if(mynpc){
       if(mynpc->getSpawn){
          mynpc->spawnPos = mynpc->pos;
          mynpc->getSpawn = false;
       }
       int maxx = mynpc->spawnPos.x + max;
    int maxy = mynpc->spawnPos.y + max;
    int minx = mynpc->spawnPos.x - max;
    int miny = mynpc->spawnPos.y - max;
       Position newPos;
       newPos.x = mynpc->pos.x;
       newPos.y = mynpc->pos.y;
       newPos.z = mynpc->pos.z;
       int wtf = random_range(1,20);
       switch(wtf){
       ...
       if((newPos.x < maxx) && (newPos.y < maxy) && (newPos.x > minx) && (newPos.y > miny))
          mynpc->doMoveTo(newPos);
       /*else
          std::cout << "Faid to doMoveTo(newPos);\n"
...
#ifdef SELL_CONTAINER
int NpcScript::luaBuyCont(lua_State *L)
{
 int cost = (int)lua_tonumber(L, -1);
 int count = (int)lua_tonumber(L, -2);
 int itemid = (int)lua_tonumber(L, -3);
 ...
 Container *backpack = dynamic_cast<Container*>(Item::CreateItem(2792));
 if(backpack)
 {
...
int NpcScript::luaIsPremium(lua_State* L)
{
	...
	if (player)
		lua_pushboolean(L, player->premmium == true);
	else
		lua_pushboolean(L, false);

	return 1;
}
int NpcScript::luaPromote(lua_State* L)
{
	int id = (int)lua_tonumber(L, -1);
	lua_pop(L, 1);
...
int NpcScript::luaGetPlayerItem(lua_State* L)
{	
	//if getPlayerItem(cid,itemid) == 1
int cid = (int)lua_tonumber(L, -2);
...
#ifdef TIJN_SHIP
int NpcScript::luaTravel(lua_State *L)
{
    //Travel(cid, cost, x, y, z)
    unsigned long id = (int)lua_tonumber(L, -5);
    unsigned int cost = (int)lua_tonumber(L, -4);
    unsigned short x = (int)lua_tonumber(L, -3);
    unsigned short y = (int)lua_tonumber(L, -2);
    unsigned char z = (int)lua_tonumber(L, -1);
    lua_pop(L,5);
    
    Npc* mynpc = getNpc(L);
    Player* player = mynpc->game->getPlayerByID(id);
    if(player) {
        if(player->substractMoney(cost)) {
            mynpc->game->teleport(player, Position(x, y, z));
            lua_pushnumber(L, 3);//succesfull
        }
        else {
            lua_pushnumber(L, 2);//not enough money
        }
    }
    else {
         lua_pushnumber(L, 1);
