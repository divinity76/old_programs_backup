...
SpellScript::SpellScript(const std::string &datadir, std::string scriptname, Spell* spell){
	...
	this->loaded=true;
	this->spell=spell;
	this->setGlobalNumber("addressOfSpell", reinterpret_cast<int>(spell));
...
int SpellScript::registerFunctions(){
    ...
    lua_register(luaState, "cancelInvisible", SpellScript::luaActionCancelInvisible);
	//lua_register(luaState, "doChallenge", SpellScript::luaSpellChallenge);
	lua_register(luaState, "makeFood", SpellScript::luaActionMakeFood);	
#ifdef CHAOS_DIJNOUTFIT
    lua_register(luaState, "transform", SpellScript::djinSpell);
#endif //CHAOS_DIJNOUTFIT
#ifdef RULY_CHALLENGELUA
    lua_register(luaState, "challenge", SpellScript::luaChallenge);
#endif //RULY_CHALLENGELUA
#ifdef RUL_DRUNK
...
#endif //BDB_UTEVO_LUX
#ifdef TALA_EXANAPOX
lua_register(luaState, "RemoveCondition", SpellScript::luaRemoveCondition);
#endif //TALA_EXANAPOX
...
Spell* SpellScript::getSpell(lua_State *L){
	...
	Spell* myspell = reinterpret_cast<Spell*>(val);
	if(!myspell){
		return 0;
	}
	return myspell;
...
int SpellScript::luaActionDoAreaSpell(lua_State *L)
{
 ...
 // if(spell->getName() == "Ultimate Healing") {
                //      spell->game->sendAnimatedText(player->pos, (int)random_range(1, 999), spell->getName());
     /*                 
    if(spell->getName() == "ultimate healing") {
                        SpectatorVec list;
...
if(spectators){
spectators->sendAnimatedText(player->pos, 199, "ExUrA ViTa"); // sending the animation
} 
}
//return;
}
*/
#ifdef TIJN_BERSERK
       if(spell->getName() == "berserk") magicArea.berserk = true;
#endif //TIJN_BERSERK
...
spell->game->changeSpeed(creature->getID(), (unsigned short)newspeed); 
...
if(attacker && creature && creature->getImmunities() != ATTACK_PARALYZE && creature->access < g_config.ACCESS_PROTECT){
...
if(attackerplayer && attackerplayer->access < g_config.ACCESS_PROTECT){
...
int SpellScript::luaActionMakeRune(lua_State *L){
	...
	lua_pop(L,1);
	   ...
	   unsigned short soul = (unsigned short)lua_tonumber(L, -1);
    lua_pop(L,1);
	...
	if(player){
		...
		int a = internalMakeRune(player,SLOT_RIGHT,spell,type,charges,soul);
		if(a == 1) {  //spend mana
			...
			int b = internalMakeRune(player,SLOT_LEFT,spell,type,charges,soul);
...
if(player){
		...
		//try to create rune 1
		a = internalMakeRune(player,SLOT_RIGHT,spell,type,charges,soul);
		if(a == 1) {
			magicTarget.manaCost = spell->getMana();
			player->soul -= soul;
		}
...
//try to create rune 2
			b = internalMakeRune(player,SLOT_LEFT,spell,type,charges,soul);
			if(b == 1) {
				magicTarget.manaCost += spell->getMana();
			}
		}
...
int SpellScript::internalMakeRune(Player *p,unsigned short sl_id,Spell *S,unsigned short id, unsigned char charges, unsigned short soul){	//check mana
	if(p->mana < S->getMana() || p->exhaustedTicks >= 1000)
		return -1;
if(p->getPlayerInfo(PLAYERINFO_SOUL) < soul)
        return -2;
...
#endif //BDB_UTEVO_LUX
#ifdef RUL_DRUNK
int SpellScript::luaActionDrunk(lua_State *L){
int ticks = (int)lua_tonumber(L, -1);
lua_pop(L,1);
long count = (long)lua_tonumber(L, -1);
lua_pop(L,1);
...
Creature* creature = spell->game->getCreatureByPosition(centerpos.x, centerpos.y, centerpos.z);
Creature* attacker = spell->game->getCreatureByID(cid);
if(attacker && creature && creature->getImmunities() != ATTACK_DRUNKNESS && creature->access < g_config.ACCESS_PROTECT){
MagicEffectTargetCreatureCondition magicCondition = MagicEffectTargetCreatureCondition(cid);
magicCondition.animationColor = 0xB4;
magicCondition.damageEffect = NM_ME_MAGIC_BLOOD;
magicCondition.hitEffect = NM_ME_MAGIC_BLOOD;
...
Player* attackerplayer = dynamic_cast<Player*>(attacker);
if(p){
p->sendTextMessage(MSG_SMALLINFO, "You are drunkness.");
p->inFightTicks = (long)g_config.getGlobalNumber("pzlocked", 0);
p->sendIcons();
}
if(attackerplayer && attackerplayer->access < g_config.ACCESS_PROTECT){
...
#endif //RUL_DRUNK
#ifdef CHAOS_DIJNOUTFIT
int SpellScript::djinSpell(lua_State *L){
    unsigned long id = (int)lua_tonumber(L, -3);
    int outfit = (int)lua_tonumber(L, -2);
    long time = (long)lua_tonumber(L, -1)*1000;
    lua_pop(L,3);
    ...
    Creature* attackedOne = spell->game->getCreatureByID(creature->attackedCreature);
    if(!attackedOne)
       return 0;
       
    attackedOne->looktype = outfit;
    spell->game->creatureChangeOutfit(attackedOne);
    
    spell->game->changeOutfitAfter(attackedOne->getID(), attackedOne->lookmaster, time);
    
    SpectatorVec list;
    SpectatorVec::iterator itc;

    spell->game->getSpectators(Range(attackedOne->pos, true), list);

    for(itc = list.begin(); itc != list.end(); ++itc) {
       Player *p = dynamic_cast<Player*>(*itc);
       if(p){
          p->sendMagicEffect(attackedOne->pos, NM_ME_MAGIC_ENERGIE);
       }
    }
    return 0;
}
#endif //CHAOS_DIJNOUTFIT
#ifdef RULY_CHALLENGELUA
int SpellScript::luaChallenge(lua_State* L)
{
    int id = (int)lua_tonumber(L, -1);
    Spell* spell = getSpell(L);
    Creature* creature = spell->game->getCreatureByID(id);  
...
int SpellScript::luaActionCancelInvisible(lua_State *L)
{
    int id = (int)lua_tonumber(L, -1);
    Spell* spell = getSpell(L);
    Creature* creature = spell->game->getCreatureByID(id);  
    ...
    spell->game->getSpectators(Range(creature->pos), list); 
    for(it = list.begin(); it != list.end(); ++it) {
        Creature* c = dynamic_cast<Creature*>(*it);
    if(c){
	spell->game->creatureChangeItemId(c,0);
	spell->game->creatureReturnToNormalOutfit(c);
    }
}
return 0;
}
#ifdef TALA_EXANAPOX
int SpellScript::luaRemoveCondition(lua_State* L)
{
    int cid = (int)lua_tonumber(L, -2);
    int condition = (int)lua_tonumber(L, -1);
    lua_pop(L,2);
    Spell* spell = getSpell(L);
    
    Creature* c = spell->game->getCreatureByID(cid);
    Player* p = c? dynamic_cast<Player*>(c) : NULL;
