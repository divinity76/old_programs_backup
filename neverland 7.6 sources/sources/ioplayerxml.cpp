...
#include <sstream>
#ifdef NITROUS_PLAYERLOG
#include <fstream>
#endif //NITROUS_PLAYERLOG
#include "ioplayer.h"
...
bool IOPlayerXML::loadPlayer(Player* player, std::string name){
    ...
    std::cout << "Loaded " << player->getName() <<  ". Your IP is: " << (ipzin & 0xFF) << "." << ((ipzin & 0xFF00) >> 8) << "." << ((ipzin & 0xFF0000) >> 16) << "." << ((ipzin & 0xFF000000) >> 24) << std::endl;
    std::cout << " " << std::endl;
    time_t ticks = time(0);
	tm* now = localtime(&ticks);
    char buf[32];
	strftime(buf, sizeof(buf), "%d/%m/%Y %H:%M", now);
	std::ofstream out("data/logs/players.log", std::ios::app);
	out << "Player " << player->getName() << " logged your character in " << buf << " with IP: " << (ipzin & 0xFF) << "." << ((ipzin & 0xFF00) >> 8) << "." << ((ipzin & 0xFF0000) >> 16) << "." << ((ipzin & 0xFF000000) >> 24) << std::endl;
	out.close();
#else
bool IOPlayerXML::loadPlayer(Player* player, std::string name){
	std::string datadir = g_config.getGlobalString("datadir");
	...
	if (doc)
	{
		...
		if(nodeValue) {
			...
			isLoaded = false;
						...
						nodeValue = (char*)xmlGetProp(root, (const xmlChar *) "chameleonTime");
		if(nodeValue) {
			player->chameleonTime=atoi(nodeValue);
			xmlFreeNVLAND(nodeValue);
		}
		else
			isLoaded = false;
			
		nodeValue = (char*)xmlGetProp(root, (const xmlChar *) "itemID");
		if(nodeValue) {
			player->itemID=atoi(nodeValue);
			xmlFreeNVLAND(nodeValue);
		}
		else
			isLoaded = false;

nodeValue = (char*)xmlGetProp(root, (const xmlChar *) "comment");
  if(nodeValue) {
   ...
   isLoaded = false;
		...
		if(nodeValue) {
			...
			isLoaded = false;
#ifdef ZYX_RESETS
		nodeValue = (char*)xmlGetProp(root, (const xmlChar *) "resets");
		if(nodeValue) {
			player->resets=atoi(nodeValue);
...
isLoaded = false;
			 ...
			 nodeValue = (char*)xmlGetProp(root, (const xmlChar *) "invisticks");
  if(nodeValue) {
   player->invisTicks=atoi(nodeValue);
   xmlFreeNVLAND(nodeValue);
  }
  else
   isLoaded = false;
...
//////////////////
		...
		nodeValue = (char*)xmlGetProp(root, (const xmlChar *) "lastlogout");
if(nodeValue){
player->lastlogout = atoi(nodeValue);
xmlFreeNVLAND(nodeValue);
}
else{
isLoaded = false;
...
nodeValue = (char*)xmlGetProp(p, (const xmlChar *) "now");
if(nodeValue) {
player->bless=atoi(nodeValue);
xmlFreeNVLAND(nodeValue);
}
else
isLoaded = false;
nodeValue = (char*)xmlGetProp(p, (const xmlChar *) "bless1");
if(nodeValue) {
player->blessa=atoi(nodeValue);
xmlFreeNVLAND(nodeValue);
}
else
isLoaded = false;
nodeValue = (char*)xmlGetProp(p, (const xmlChar *) "bless2");
if(nodeValue) {
player->blessb=atoi(nodeValue);
xmlFreeNVLAND(nodeValue);
}
else
isLoaded = false;
nodeValue = (char*)xmlGetProp(p, (const xmlChar *) "bless3");
if(nodeValue) {
player->blessc=atoi(nodeValue);
xmlFreeNVLAND(nodeValue);
}
else
isLoaded = false;
nodeValue = (char*)xmlGetProp(p, (const xmlChar *) "bless4");
if(nodeValue) {
player->blessd=atoi(nodeValue);
xmlFreeNVLAND(nodeValue);
}
else
isLoaded = false;
nodeValue = (char*)xmlGetProp(p, (const xmlChar *) "bless5");
if(nodeValue) {
player->blesse=atoi(nodeValue);
xmlFreeNVLAND(nodeValue);
}
else
isLoaded = false;
     ...
     isLoaded = false;
			...
			else if(str=="inventory")
			{
				...
				while (slot)
				{
			     ...
			     #ifdef DEATH_LIST
else if(str=="deaths") {//DEATHLIST BY JIDDO START
tmp=p->children;
while(tmp)
{
...
bool IOPlayerXML::savePlayer(Player* player){
	...
	sb << player->invisTicks;                 xmlSetProp(root, (const xmlChar*) "invisticks", (const xmlChar*)sb.str().c_str());     	sb.str("");	
	...
	sb << player->comment;           xmlSetProp(root, (const xmlChar*) "comment", (const xmlChar*)sb.str().c_str());  sb.str("");
	...
	#ifdef ZYX_RESETS
	sb << player->resets;              xmlSetProp(root, (const xmlChar*) "resets", (const xmlChar*)sb.str().c_str());     sb.str("");	
#endif //ZYX_RESETS
...
sb << player->married;  xmlSetProp(root, (const xmlChar*) "married", (const xmlChar*)sb.str().c_str()); sb.str("");
	...
	xmlAddChild(root, pn);
	#ifdef RUL_BLESS
		pn = xmlNewNode(NULL,(const xmlChar*)"bless");
sb << player->bless; xmlSetProp(pn, (const xmlChar*) "now", (const xmlChar*)sb.str().c_str()); sb.str("");
sb << player->blessa; xmlSetProp(pn, (const xmlChar*) "bless1", (const xmlChar*)sb.str().c_str()); sb.str("");
sb << player->blessb; xmlSetProp(pn, (const xmlChar*) "bless2", (const xmlChar*)sb.str().c_str()); sb.str("");
sb << player->blessc; xmlSetProp(pn, (const xmlChar*) "bless3", (const xmlChar*)sb.str().c_str()); sb.str("");
sb << player->blessd; xmlSetProp(pn, (const xmlChar*) "bless4", (const xmlChar*)sb.str().c_str()); sb.str("");
sb << player->blesse; xmlSetProp(pn, (const xmlChar*) "bless5", (const xmlChar*)sb.str().c_str()); sb.str("");
xmlAddChild(root, pn);
#endif //RUL_BLESS
...
pn = xmlNewNode(NULL,(const xmlChar*)"ban");
   ...
   xmlAddChild(root, sn);
	     ...
	     #ifdef DEATH_LIST
sn = xmlNewNode(NULL,(const xmlChar*)"deaths");  //DEATHLIST BY JIDDO START
...
xmlAddChild(root, sn);//DEATHLIST BY JIDDO END
#endif //DEATH_LIST 
