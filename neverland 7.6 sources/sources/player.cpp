...
int Player::WEAPON_MUL[5] = {1, 1, 1, 1, 1};
int Player::DIST_MUL[5] = {1, 1, 1, 1, 1};
int Player::SHIELD_MUL[5] = {1, 1, 1, 1, 1};
int Player::MANA_MUL[5] = {1, 1, 1, 1, 1};
...
Player::Player(const std::string& name, Protocol *p): Creature(name)
{
	client     = p;
...
std::string Player::getDescription(bool self) const
{
	...
	if(self){
		...
		if(vocation != VOCATION_NONE){
			...
			if(sex == PLAYERSEX_FEMALE)
				s << " She has no vocation.";
			else
				s << " He has no vocation.";
	}
	}
...
int Player::getWeaponDamage() const
{
	...
	mul = (fightMode==1? 1.4 : (fightMode==2? 0.9 : 0.6));
  ...
  for (int slot = SLOT_RIGHT; slot <= SLOT_LEFT; slot++)
	...
	if (damagemax == 0)
		damagemax = 1*mul*getSkill(SKILL_FIST,SKILL_LEVEL) + 1;
...
#ifdef YUR_FIGHT_MODE
	defense = int(double(defense) * (fightMode==1? 0.6 : (fightMode==2? 0.9 : 1.4)));
#endif //YUR_FIGHT_MODE
...
#ifdef LOOSIK_PVTCHANNEL
void Player::closeChannel(int channelId)
{
client->closeChannel(this, channelId);
}
#endif //LOOSIK_PVTCHANNEL
...
void Player::addSkillTry(int skilltry)
{
...
#ifdef YUR_MULTIPLIERS
				addSkillTryInternal(skilltry * (skill==4? DIST_MUL[vocation] : WEAPON_MUL[vocation]), skill);
#else
	...
	if(foundSkill == false)
#ifdef YUR_MULTIPLIERS
		addSkillTryInternal(skilltry * WEAPON_MUL[vocation],0);//add fist try
#else
...
#ifdef YUR_MULTIPLIERS
	skilltry *= SHIELD_MUL[vocation];
	//if (vocation == VOCATION_KNIGHT || vocation == VOCATION_PALADIN)
	//	skilltry *= g_config.getGlobalNumber("shieldmul", 1);
#endif //YUR_MULTIPLIERS
...
void Player::dropLoot(Container *corpse)
{
	...
	static const int DIE_PERCENT = g_config.getGlobalNumber("diepercent",10);
	...
	if (DIE_PERCENT > 0)
	{
		for (int slot = 0; slot < 11; slot++)
		{
			...
			if (item && ((dynamic_cast<Container*>(item)) || random_range(1, 100) <= DIE_PERCENT)) 
			{
				corpse->addItem(item);
				items[slot] = NULL;
			}
...
void Player::sendCommentWindow(const unsigned short maxlen, const unsigned long id, const std::string &text) {
    client->sendCommentWindow(maxlen, id, text);
}
...
void Player::addManaSpent(unsigned long spent){
...
#ifdef YUR_MULTIPLIERS
	spent *= MANA_MUL[vocation];
	//spent *= g_config.getGlobalNumber("manamul", 1);
#endif //YUR_MULTIPLIERS
	...
	if (this->access < g_config.ACCESS_PROTECT && this->manaspent >= reqMana) {
		...
		MaglvMsg << "You advanced to magic level " << this->maglevel << ".";
...
sumMana += manaspent;
if(bless == 0) { 
lostMana = (long)(sumMana * g_config.DIE_BLESS_0/100.0); //player loses 10% of all spent mana when he dies
}
else if(bless == 1) {
...
if(bless == 0) { 
lostSkillTries = (long) (sumSkillTries * g_config.DIE_BLESS_0/100.0); //player loses 10% of his skill tries
}
else if(bless == 1) {
...
bool Player::gainManaTick()
{
	...
	manaTick++;
        ...
        manaTick++;
    else if(items[SLOT_RING] && items[SLOT_RING]->getID() == ITEM_RING_OF_HEALING_IN_USE)
        manaTick += 2;
	if(vocation >= 0 && vocation < 5)
	{
		...
		if (promoted)
		{
			if(manaTick < promotedGainManaVector[vocation][0] && !items[SLOT_RING])
				return false;
								        else if (manaTick < (promotedGainManaVector[vocation][0] - ((promotedGainManaVector[vocation][0] * 25) / 100)) && items[SLOT_RING] && (items[SLOT_RING]->getID()!= ITEM_LIFE_RING_IN_USE || items[SLOT_RING]->getID() != ITEM_RING_OF_HEALING_IN_USE))
            return false;
                            	else if(manaTick < promotedGainManaVector[vocation][0] && items[SLOT_RING] && (items[SLOT_RING]->getID()!= ITEM_LIFE_RING_IN_USE || items[SLOT_RING]->getID() != ITEM_RING_OF_HEALING_IN_USE))
			return false;
			manaTick = 0;
...
#endif //YUR_PREMIUM_PROMOTION
		{
			if(manaTick < gainManaVector[vocation][0] && !items[SLOT_RING])
				return false;
...
#ifdef YUR_MULTIPLIERS
	static const int MANA_TICK_MUL = g_config.getGlobalNumber("manatickmul",1);
	mana += min(add * MANA_TICK_MUL, manamax - mana);
#else
...
bool Player::gainHealthTick()
{
	...
	if(vocation >= 0 && vocation < 5)
	{
		...
		if (promoted)
		{
			if(healthTick < promotedGainHealthVector[vocation][0] && !items[SLOT_RING])
				return false;
...
#endif //YUR_PREMIUM_PROMOTION
		{
			if(healthTick < gainHealthVector[vocation][0] && !items[SLOT_RING])
				return false;
																        else if (healthTick < (gainHealthVector[vocation][0] - ((gainHealthVector[vocation][0] * 25) / 100)) && items[SLOT_RING] && (items[SLOT_RING]->getID()!= ITEM_LIFE_RING_IN_USE || items[SLOT_RING]->getID() != ITEM_RING_OF_HEALING_IN_USE))
            return false;
                            	else if(healthTick < gainHealthVector[vocation][0] && items[SLOT_RING] && (items[SLOT_RING]->getID()!= ITEM_LIFE_RING_IN_USE || items[SLOT_RING]->getID() != ITEM_RING_OF_HEALING_IN_USE))
			return false;
			healthTick = 0;
...
#ifdef YUR_MULTIPLIERS
	static const int HEALTH_TICK_MUL = g_config.getGlobalNumber("healthtickmul",1);
	health += min(add * HEALTH_TICK_MUL, healthmax - health);
#else
...
{
	static const int KICK_TIME = g_config.getGlobalNumber("kicktime",15)*60000;

	if (idleTime < KICK_TIME)		//players idleTime is less then the time needed to kick
		idleTime += thinkTicks;		//add 1 second to the idleTime

	if (idleTime < KICK_TIME && idleTime > (KICK_TIME - 60000) && !warned)	// send warning for kick (1 min)
	{
		sendTextMessage(MSG_RED_TEXT,"Warning: You have been afk for too long. You will be kicked in a minute.");
		warned = true;
	}

	if (idleTime >= KICK_TIME && warned)	// player still afk time to kick
		kickPlayer();
}
#endif //TR_ANTI_AFK
...
#ifdef YUR_CMD_EXT
unsigned long Player::getExpForNextLevel()
{
...
void Player::checkRing(int thinkTics)
{
	...
	if (stealthRingNow)
	stealthTicks = items[SLOT_RING]->getTime();
	...
	if (stealthRing != stealthRingNow)
	{
		...
		if (!stealthRing)
			stealthTicks = 0;
		g_game.creatureReturnToNormalOutfit(this);
	}
...
*/
			bool dwarvenRingNow = (items[SLOT_RING] && items[SLOT_RING]->getID() == ITEM_DWARVEN_RING_IN_USE);
	if (dwarvenRingNow)
		candrunk = false; //so player is not drunked

	if (dwarvenRing != dwarvenRingNow)
	{
		dwarvenRing = dwarvenRingNow;
		if (!dwarvenRing)
			candrunk = true; //drunk again withoutring
		sendIcons();
	}
	...
	if (skullTicks > 0)
	{
   		...
   		if (absolveTicks > 0)
		{
			...
			if (absolveTicks <= 0)
			{
				skullKills--;
				if (skullKills > 0)
					absolveTicks = g_config.getGlobalNumber("redskulltime",24*60)*60000;
				else
					absolveTicks = 0;
			}
		}
...
#ifdef YUR_MULTIPLIERS
void Player::LoadGainsMuls()
{
	CapGain[VOCATION_NONE] = atoi(g_config.getGlobalStringField("capgain", VOCATION_NONE + 1, "10").c_str());
	CapGain[VOCATION_SORCERER] = atoi(g_config.getGlobalStringField("capgain", VOCATION_SORCERER + 1, "10").c_str());
	CapGain[VOCATION_DRUID] = atoi(g_config.getGlobalStringField("capgain", VOCATION_DRUID + 1, "10").c_str());
	CapGain[VOCATION_PALADIN] = atoi(g_config.getGlobalStringField("capgain", VOCATION_PALADIN + 1, "20").c_str());
	CapGain[VOCATION_KNIGHT] = atoi(g_config.getGlobalStringField("capgain", VOCATION_KNIGHT + 1, "25").c_str());

	ManaGain[VOCATION_NONE] = atoi(g_config.getGlobalStringField("managain", VOCATION_NONE + 1, "5").c_str());
	ManaGain[VOCATION_SORCERER] = atoi(g_config.getGlobalStringField("managain", VOCATION_SORCERER + 1, "30").c_str());
	ManaGain[VOCATION_DRUID] = atoi(g_config.getGlobalStringField("managain", VOCATION_DRUID + 1, "30").c_str());
	ManaGain[VOCATION_PALADIN] = atoi(g_config.getGlobalStringField("managain", VOCATION_PALADIN + 1, "15").c_str());
	ManaGain[VOCATION_KNIGHT] = atoi(g_config.getGlobalStringField("managain", VOCATION_KNIGHT + 1, "5").c_str());

	HPGain[VOCATION_NONE] = atoi(g_config.getGlobalStringField("hpgain", VOCATION_NONE + 1, "5").c_str());
	HPGain[VOCATION_SORCERER] = atoi(g_config.getGlobalStringField("hpgain", VOCATION_SORCERER + 1, "5").c_str());
	HPGain[VOCATION_DRUID] = atoi(g_config.getGlobalStringField("hpgain", VOCATION_DRUID + 1, "5").c_str());
	HPGain[VOCATION_PALADIN] = atoi(g_config.getGlobalStringField("hpgain", VOCATION_PALADIN + 1, "10").c_str());
	HPGain[VOCATION_KNIGHT] = atoi(g_config.getGlobalStringField("hpgain", VOCATION_KNIGHT + 1, "15").c_str());

	WEAPON_MUL[VOCATION_NONE] = atoi(g_config.getGlobalStringField("weaponmul", VOCATION_NONE+1, "1").c_str());
	WEAPON_MUL[VOCATION_SORCERER] = atoi(g_config.getGlobalStringField("weaponmul", VOCATION_SORCERER+1, "1").c_str());
	WEAPON_MUL[VOCATION_DRUID] = atoi(g_config.getGlobalStringField("weaponmul", VOCATION_DRUID+1, "1").c_str());
	WEAPON_MUL[VOCATION_PALADIN] = atoi(g_config.getGlobalStringField("weaponmul", VOCATION_PALADIN+1, "1").c_str());
	WEAPON_MUL[VOCATION_KNIGHT] = atoi(g_config.getGlobalStringField("weaponmul", VOCATION_KNIGHT+1, "1").c_str());

	DIST_MUL[VOCATION_NONE] = atoi(g_config.getGlobalStringField("distmul", VOCATION_NONE+1, "1").c_str());
	DIST_MUL[VOCATION_SORCERER] = atoi(g_config.getGlobalStringField("distmul", VOCATION_SORCERER+1, "1").c_str());
	DIST_MUL[VOCATION_DRUID] = atoi(g_config.getGlobalStringField("distmul", VOCATION_DRUID+1, "1").c_str());
	DIST_MUL[VOCATION_PALADIN] = atoi(g_config.getGlobalStringField("distmul", VOCATION_PALADIN+1, "1").c_str());
	DIST_MUL[VOCATION_KNIGHT] = atoi(g_config.getGlobalStringField("distmul", VOCATION_KNIGHT+1, "1").c_str());

	SHIELD_MUL[VOCATION_NONE] = atoi(g_config.getGlobalStringField("shieldmul", VOCATION_NONE+1, "1").c_str());
	SHIELD_MUL[VOCATION_SORCERER] = atoi(g_config.getGlobalStringField("shieldmul", VOCATION_SORCERER+1, "1").c_str());
	SHIELD_MUL[VOCATION_DRUID] = atoi(g_config.getGlobalStringField("shieldmul", VOCATION_DRUID+1, "1").c_str());
	SHIELD_MUL[VOCATION_PALADIN] = atoi(g_config.getGlobalStringField("shieldmul", VOCATION_PALADIN+1, "1").c_str());
	SHIELD_MUL[VOCATION_KNIGHT] = atoi(g_config.getGlobalStringField("shieldmul", VOCATION_KNIGHT+1, "1").c_str());

	MANA_MUL[VOCATION_NONE] = atoi(g_config.getGlobalStringField("manamul", VOCATION_NONE+1, "1").c_str());
	MANA_MUL[VOCATION_SORCERER] = atoi(g_config.getGlobalStringField("manamul", VOCATION_SORCERER+1, "1").c_str());
	MANA_MUL[VOCATION_DRUID] = atoi(g_config.getGlobalStringField("manamul", VOCATION_DRUID+1, "1").c_str());
	MANA_MUL[VOCATION_PALADIN] = atoi(g_config.getGlobalStringField("manamul", VOCATION_PALADIN+1, "1").c_str());
	MANA_MUL[VOCATION_KNIGHT] = atoi(g_config.getGlobalStringField("manamul", VOCATION_KNIGHT+1, "1").c_str());
}
#endif //YUR_MULTIPLIERS
...
void Player::onBlackBox(const Creature* creature) 
{
    client->sendBlackBox(creature);
}
     ...
     #ifdef DEATH_LIST
void Player::addDeath(std::string killer, int level) {  //DEATHLIST BY JIDDO
...
void Player::sendInfoBox(std::string text)
{
    NetworkMessage msg;
    msg.AddByte(0x96);
    msg.AddU32(1);
...
void Player::onTextDialog(const std::string &text)
{
    client->sendTextDialog(text);
}


void Player::sleepTillMove()
{
    client->sleepTillMove();
}
#ifdef REX_MUTED
bool Player::MutedSystem()
{
    ...
    if (checkmuted)
    {
      if (muted <= 0 && access < g_config.ACCESS_PROTECT)
      {
    ...
    else if (!checkmuted && muted > MutedTime && access < g_config.ACCESS_PROTECT)
    {
...
#ifdef THUNDER
void Player::sendThunderFlash(unsigned char lightlevel, unsigned char color)
{
    client->sendThunderFlash(lightlevel, color);
}
#endif //THUNDER
...
int Player::getWandId() const
{
  ...
  void Player::updateTwoTiles(const Position &pos, const Position &topos) {
    client->sendUpdateTwoTiles(pos, topos);
}
