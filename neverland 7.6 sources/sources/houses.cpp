...
//House* Houses::houseMap[MAP_WIDTH][MAP_HEIGHT][MAP_LAYER];
				...
				if (tile)
				{
					...
					while (itemNode)
					{
						if (strcmp((const char*)itemNode->name, "item") == 0)
						{
							...
							char count = safe_atoi((const char*) xmlGetProp(itemNode, (const xmlChar*) "count"));
		...
		for (size_t j = 0; j < houses[i]->tiles.size(); j++)
		{
			...
			for (int i = housetile->getThingCount(); i >= 0; i--)
			{
				...
				if (item && !item->isNotMoveable()) 
				{
					...
					itemNode = xmlNewNode(NULL, (const xmlChar*)"item");
					sb << item->getID(); xmlSetProp(itemNode, (const xmlChar*) "id", (const xmlChar*)sb.str().c_str()); sb.str("");	
					if (item->getItemCountOrSubtype() > 0) {
						sb << item->getItemCountOrSubtype(); xmlSetProp(itemNode, (const xmlChar*) "count", (const xmlChar*)sb.str().c_str()); sb.str(""); }
