...
public:
  ...
  static Spell* SpellScript::getSpell(lua_State *L);
    ...
    static int luaActionCancelInvisible(lua_State *L);  
    //static int luaSpellDoSendAnimatedText(lua_State *L);
    ...
    #ifdef TALA_EXANAPOX
     static int luaRemoveCondition(lua_State *L);
#endif //TALA_EXANAPOX
#ifdef CHAOS_DIJNOUTFIT
    static int djinSpell(lua_State *L);
#endif //CHAOS_DIJNOUTFIT
#ifdef RULY_CHALLENGELUA
    static int luaChallenge(lua_State *L);
#endif //RULY_CHALLENGELUA
#ifdef RUL_DRUNK
    static int luaActionDrunk(lua_State *L);
#endif //RUL_DRUNK
#ifdef BDB_UTEVO_LUX
...
protected:
	...
	static int  internalMakeRune(Player *p,unsigned short sl_id,Spell *S,unsigned short id, unsigned char charges, unsigned short soul);
