...
*/
        ...
        if(ground == GROUND_WATER || ground == GROUND_SWAMP || ground == GROUND_GRASS)
        ground = generateRandom(ground); //Dunno if the = thing is needed..
...
unsigned short IOMapXML::generateRandom(unsigned short id)
{
      int change = 0;
      switch(id) // I didnt want the grass change so often as water and swamp..
      {
              case GROUND_WATER:
              case GROUND_SWAMP:
      ...
      if(change == 1)
      {
               change = rand()%15; //All those ids got about 15 extra tiles..
