...
Creature(name)
{
	...
	if (doc){
		...
		if(nodeValue) {
			pushable = (atoi(nodeValue) != 0);
			xmlFreeNVLAND(nodeValue);
		}
		
		nodeValue = (char*)xmlGetProp(root, (const xmlChar *)"bossmonster");
		if(nodeValue) {
			bossmonster = (atoi(nodeValue) != 0);
			xmlFreeNVLAND(nodeValue);
		}

		nodeValue = (char*)xmlGetProp(root, (const xmlChar *)"poisonmonster");
		if(nodeValue) {
			poisonmonster = (atoi(nodeValue) != 0);
			xmlFreeNVLAND(nodeValue);
		}
...
#ifdef BDB_BLOOD
		nodeValue = (char*)xmlGetProp(root, (const xmlChar *)"blood");
		if(nodeValue) {
			bloodcolor = atoi(nodeValue);
			xmlFreeNVLAND(nodeValue);
		}
#endif //BDB_BLOOD
...
if(strcmp(attacktype.c_str(), "poison_melee") == 0){
							PhysicalAttackClass* physicalattack = new PhysicalAttackClass();

							physicalattack->fighttype = FIGHT_POISON_MELEE;
							...
							nodeValue = (char*)xmlGetProp(tmp, (const xmlChar *)"maxhits");
							if(nodeValue) {
								physicalattack->maxHits = atoi(nodeValue);
								xmlFreeNVLAND(nodeValue);
							}
                            
							nodeValue = (char*)xmlGetProp(tmp, (const xmlChar *)"maxhitsdamage");
							if(nodeValue) {
								physicalattack->maxHitsDamage = atoi(nodeValue);
								xmlFreeNVLAND(nodeValue);
							}                            							
							
							physicalAttacks[physicalattack] = TimeProbabilityClass(cycleTicks, probability, exhaustionTicks);
						}	
						if(strcmp(attacktype.c_str(), "fire_melee") == 0){
							PhysicalAttackClass* physicalattack = new PhysicalAttackClass();

							physicalattack->fighttype = FIGHT_FIRE_MELEE;
						...
						if(strcmp(attacktype.c_str(), "energy_melee") == 0){
							PhysicalAttackClass* physicalattack = new PhysicalAttackClass();

							physicalattack->fighttype = FIGHT_ENERGY_MELEE;
						...
						else if(strcmp(attacktype.c_str(), "distance") == 0)
					 {
							...
							if(spells.getAllSpells()->find(spellname) != spells.getAllSpells()->end())
						 {
								instantSpells[spellname].push_back(timeprobsystem);
												...
												else if(strcmp(attacktype.c_str(), "costum") == 0) {
                            CostumSpellClass* spell = new CostumSpellClass();
                            
                            nodeValue = (char*)xmlGetProp(tmp, (const xmlChar *)"name");
                            if(nodeValue) {
								spell->name = nodeValue;
								xmlFreeNVLAND(nodeValue);
							}
							
							nodeValue = (char*)xmlGetProp(tmp, (const xmlChar *)"mindamage");
							if(nodeValue) {
								spell->minDamage = (unsigned int)atoi(nodeValue);
								xmlFreeNVLAND(nodeValue);
							}
							
							nodeValue = (char*)xmlGetProp(tmp, (const xmlChar *)"maxdamage");
							if(nodeValue) {
								spell->maxDamage = (unsigned int)atoi(nodeValue);
								xmlFreeNVLAND(nodeValue);
							}
							
                            //costumspells[spell] = TimeProbabilityClass(cycleTicks, probability, exhaustionTicks);
                            costumspells[spell].push_back(timeprobsystem);
                        }
				...
				while(tmp)
			 {
					if (strcmp((const char*)tmp->name, "defense") == 0) {
						...
						if(strcmp(immunity.c_str(), "energy") == 0)
							...
							else if(strcmp(immunity.c_str(), "invis") == 0)
							immunities |= ATTACK_INVIS;
					}
...
bool Monster::isCreatureAttackable(const Creature* creature)
{
	...
	if(!dynamic_cast<const Player*>(creature) && !(const_cast<Creature*>(creature))->getMaster())
		return false;
		#ifdef TIJN_UTANAVID
				 //   if(creature->getItemId() == 0 && (ATTACK_INVIS & getImmunities()) != ATTACK_INVIS)
       // return false;
#endif //TIJN_UTANAVID
...
if(target->getItemId() == 0 && (ATTACK_INVIS & getImmunities()) != ATTACK_INVIS)
       return false;
       
	//if (target->isInvisible() && !(immunities & ATTACK_INVIS))
	//	return false;
...
bool Monster::isCreatureReachable(const Creature* creature)
{
     if(creature->getItemId() == 0 && (ATTACK_INVIS & getImmunities()) != ATTACK_INVIS)
       return false;
	//if (creature->isInvisible() && !(immunities & ATTACK_INVIS))
	//return false;
...
#ifdef NAXE_CANTWALK
if(state != STATE_IDLE && !(state == STATE_IDLESUMMON && hasLostMaster) && this->name != "Spit Nettle" && this->name != "Magicthrower"  && this->name != "Deathslicer" && this->name != "Plaguethrower") {
#else
	//process movement
...
void Monster::reThink(bool updateOnlyState /* = true*/)
{
	if(isSummon()) {
		...
		if(state == STATE_ATTACKING) {
			//summons
			if(summons.size() < (size_t)maxSummons) {
 ...
 #ifdef TR_SUMMONFIX
                       summonPos.x += random_range(-4,4);
                       summonPos.y += random_range(-4,4);
 #endif //TR_SUMMONFIX
...
#ifdef CAYAN_POISONMELEE
if((paIt->first->fighttype == FIGHT_MELEE) || (paIt->first->fighttype == FIGHT_POISON_MELEE) || (paIt->first->fighttype == FIGHT_FIRE_MELEE) || (paIt->first->fighttype == FIGHT_ENERGY_MELEE)){
				hasmelee = true;
				break;
            }
#else
...
#ifdef CAYAN_POISONMELEE
if(hasmelee && paIt->first->fighttype == FIGHT_POISON_MELEE) {
				curPhysicalAttack = paIt->first;
				game->creatureMakeDamage(this, attackedCreature, getFightType());
				if((attackedCreature->getImmunities()) != ATTACK_POISON){
                    ...
                    if(probability > 45){
                        for(Conditions::iterator condIt = conditions.begin(); condIt != conditions.end(); ++condIt) {
                            ...
                            if(condIt->first == ATTACK_POISON)
                                attackedCreature->conditions.erase(condIt);
                        }
    	    			game->PoisonMelee(attackedCreature, this, 19, NM_ME_POISEN_RINGS, NM_ME_POISEN_RINGS, ATTACK_POISON, true, curPhysicalAttack->maxHitsDamage, curPhysicalAttack->maxHitsDamage, (long)g_config.getGlobalNumber("exhausted", 1500), curPhysicalAttack->maxHits);
                    }
                    Player *player = dynamic_cast<Player*>(attackedCreature);
                    if(player)
                        player->sendIcons();    	    			
			...
			else if(hasmelee && paIt->first->fighttype == FIGHT_FIRE_MELEE) {
				curPhysicalAttack = paIt->first;
				game->creatureMakeDamage(this, attackedCreature, getFightType());
				if((attackedCreature->getImmunities()) != ATTACK_FIRE){
			    	int probability = random_range(1, 100);
                    if(probability > 45){
                        for(Conditions::iterator condIt = conditions.begin(); condIt != conditions.end(); ++condIt) {
                            Player* player = dynamic_cast<Player*>(attackedCreature);
                            if(condIt->first == ATTACK_FIRE)
                                attackedCreature->conditions.erase(condIt);
                        }                                   
    	    			game->PoisonMelee(attackedCreature, this, 199, NM_ME_HITBY_FIRE, NM_ME_HITBY_FIRE, ATTACK_FIRE, true, curPhysicalAttack->maxHitsDamage, curPhysicalAttack->maxHitsDamage, (long)g_config.getGlobalNumber("exhausted", 1500), curPhysicalAttack->maxHits);
                    }
                    Player *player = dynamic_cast<Player*>(attackedCreature);
                    if(player)
                        player->sendIcons();    	    			
			...
			else if(hasmelee && paIt->first->fighttype == FIGHT_ENERGY_MELEE) {
				curPhysicalAttack = paIt->first;
				game->creatureMakeDamage(this, attackedCreature, getFightType());
				if((attackedCreature->getImmunities()) != ATTACK_ENERGY){
			    	int probability = random_range(1, 100);
                    if(probability > 45){
                        for(Conditions::iterator condIt = conditions.begin(); condIt != conditions.end(); ++condIt) {
                            Player* player = dynamic_cast<Player*>(attackedCreature);
                            if(condIt->first == ATTACK_ENERGY)
                                attackedCreature->conditions.erase(condIt);
                        }                    
    	    			game->PoisonMelee(attackedCreature, this, 71, NM_ME_ENERGY_DAMAGE, NM_ME_ENERGY_DAMAGE, ATTACK_ENERGY, true, curPhysicalAttack->maxHitsDamage, curPhysicalAttack->maxHitsDamage, (long)g_config.getGlobalNumber("exhausted", 1500), curPhysicalAttack->maxHits);
                    }
                    Player *player = dynamic_cast<Player*>(attackedCreature);
                    if(player)
                        player->sendIcons();    	    			
			...
			else if(!hasmelee || (hasmelee && paIt->first->fighttype == FIGHT_MELEE)) {
				curPhysicalAttack = paIt->first;
				game->creatureMakeDamage(this, attackedCreature, getFightType());
			}
#else
			...
			for(CostumSpells::iterator csIt = costumspells.begin(); csIt != costumspells.end(); ++csIt) {
            for(TimeProbabilityClassVec::iterator asIt = csIt->second.begin(); asIt != csIt->second.end(); ++asIt) {
                TimeProbabilityClass& timeprobsystem = *asIt;
				if(timeprobsystem.onTick(500) || (random_range(1, 100) <= modeProb)) {
                    bool success = false;
                    #ifdef TIJN_DRAIN
                    if(csIt->first->name == "manadrain") {
                        if(isInRange(attackedCreature->pos)) {
                            success = game->drain(1, attackedCreature, this, csIt->first->minDamage, csIt->first->maxDamage);
                        }
                    }
                    else if(csIt->first->name == "lifedrain") {
                        success = game->drain(2, attackedCreature, this, csIt->first->minDamage, csIt->first->maxDamage);
                    }
                    #endif //TIJN_DRAIN
                    #ifdef TIJN_UTANAVID
                                       else if(csIt->first->name == "invis") {
                        success = game->creatureChangeItemId(this, 0);
                        if(this->invisTicks == 0 && success) {
                            this->invisTicks = 300;
                             }
                              }
#endif //TIJN_UTANAVID
...
else if(!this->isSummon() && (abs(this->pos.x-this->masterPos.x) > game->distanceToKill || 
    abs(this->pos.y-this->masterPos.y) > game->distanceToKill)){    
...
#ifdef TIJN_UTANAVID
void Monster::onCreatureChangeOutfit(const Creature* creature)
{
    if(creature->getItemId() == 99) {
        ...
        if(creature->getItemId() == 0) {
            if((ATTACK_INVIS & getImmunities()) != ATTACK_INVIS) {
                onCreatureLeave(creature);
            }
        }
    }
...
void Monster::changeTarget(const Creature* target) {
bool canReach = isCreatureReachable(target);
selectTarget(target, canReach);
}
