...
#if __DEBUG__
	//for debug
<< getName() << ", has the vocation: " << (int)vocation << " and is training his " << getSkillName(skill) << "(" << skill << "). Tries: " << skills[skill][SKILL_TRIES] << "(" << getReqSkillTries(skill, (skills[skill][SKILL_LEVEL] + 1), vocation) << ")" << std::endl; cout << "Current skill: " << skills[skill][SKILL_LEVEL] << std::endl; 
#endif
 //Need skill up?
 if (skills[skill][SKILL_TRIES] >= getReqSkillTries(skill, (skills[skill][SKILL_LEVEL] + 1), vocation)) {
  ...
  g_game.sendAnimatedTextExt(this->pos, g_config.getGlobalNumber("anicolor", 180), ani.str().c_str());
 }
	else{
	 //update percent
...
{
#ifdef __DEBUG__
cout << getName() << ", addContainer: " << (int)containerid << std::endl; 
#endif
	if(containerid > 0xF)
		return;
...
#ifdef __DEBUG__
	cout << getName() << ", closeContainer: " << (int)containerid << std::endl;
#endif
}
...
void Player::addManaSpent(unsigned long spent){
 ...
 if (this->access == 0 && this->manaspent >= reqMana) {
  ...
  this->sendStats();
  if (g_config.ADVANCE_ANI)
  g_game.sendAnimatedTextExt(this->pos, g_config.getGlobalNumber("anicolor", 180), "Magic up!");
 }
...
void Player::sendInfoBox(std::string text, int itemid)
{
     client->sendInfoBox(text, itemid);
}
