...
if(item->isClosedLevelDoor())
{
int x;
bool found = false;
for(int i = 0; i < game->levelDoors.size(); i++)
{
if(game->levelDoors[i].second == pos)
{
x = i;
found = true;
break;
...
if(!found)
return false;
if(player->level < game->levelDoors[x].first && player->access == 0)
{
std::stringstream insuficientLevel;
insuficientLevel << "Your level is too low." << " You need level " << game->levelDoors[x].first << " to pass this door.";
player->sendTextMessage(MSG_INFO, insuficientLevel.str().c_str());
return false;
}
Position doorPos = game->levelDoors[x].second;
if(doorPos == player->pos)
return false;
int id = itemid+1;
int ido = itemid;
item->setID(itemid+1);
...
if((ido == 1227 || ido == 1245 || itemid == 1259) && player->pos.x == doorPos.x+1)
{
game->thingMove(player, player, doorPos.x, doorPos.y, doorPos.z, 1);
game->walkdoors(player, doorPos.x-1, doorPos.y, doorPos.z, item);
...
if((ido == 1229 || ido == 1247 || itemid == 1261) && player->pos.y == doorPos.y-1)
{
game->thingMove(player, player, doorPos.x, doorPos.y, doorPos.z, 1);
game->walkdoors(player, doorPos.x, doorPos.y+1, doorPos.z, item);
int ticks = (int)player->getSleepTicks();
}
if((ido == 1229 || ido == 1247 || itemid == 1261) && player->pos.y == doorPos.y+1)
{
...
int ActionScript::luaActionDoAddTeleport(lua_State *L)
{
    ...
    Item *newitem = Item::CreateItem(1387);
