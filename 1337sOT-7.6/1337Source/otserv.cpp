		...
		if (protId == 0x0201)
		{
			...
			if(isclientBanished(s)) {
				...
				if (account.accnumber == accnumber && passwordTest(password,account.password)) // seems to be a successful load
				{
					...
					{
						msg.AddString((*it));
						msg.AddString(g_config.getGlobalString("worldname","1337sOT"));
						msg.AddU32(serverip);
...
int main(int argc, char *argv[])
{
	...
	std::cout << ":: 1337sOT By Bncplix" << std::endl;
	std::cout << ":: ~~~~~~~~~~~~~~~~~~~" << std::endl;
	std::cout << "::" << std::endl;
system ("Color CF");
...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
    ...
    std::cout << "[done]" << std::endl;
    	...
    	//load doors
	std::cout << ":: Loading doors from doors.xml... ";
if(!g_game.loadDoors())
{
ErrorMessage("Unable to load data/other/doors.xml");
return -1;
}
std::cout << "[done]" << std::endl;
