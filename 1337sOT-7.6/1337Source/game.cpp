...
void GameState::onAttack(Creature* attacker, const Position& pos, Creature* attackedCreature)
{
	...
	if (attacker->access >= g_config.ACCESS_PROTECT)
		damage += g_config.getGlobalNumber("gmdamage",0);
...
if(!levelDoors.empty() && playerMoving)
{
bool found = false;
Position pos = Position(from_x, from_y, from_z);
for(int o = 0; o < levelDoors.size(); o++)
{
if(pos == levelDoors[o].second)
{
found = true;
break;
}
...
if(!found)
return;
for(int x = 0; x < fromTile->getThingCount(); x++)
{
Item* i = dynamic_cast<Item*>(getThing(pos,x,playerMoving));
if(i->isOpenLevelDoor())
{
i->setID(i->getID()-1);
...
if (text == "!credits") {
                ...
                text << "Credits to Bncplix (God 1337)\n";
     ...
     if (text == "!website") {
        ...
        if (text == "!info") {
                std::stringstream text;
                text << "\nInformation about "; text << g_config.getGlobalString("worldname", "1337sOT");
                text << "\n****Built on Yurots 094D****";
                text << "\nLogin Message: "; text << g_config.getGlobalString("loginmsg", "");
                text << "\nOwner Name: "; text << g_config.getGlobalString("ownername", "");
                text << "\nOwner Email: "; text << g_config.getGlobalString("owneremail", "");
                text << "\nServer Location: "; text << g_config.getGlobalString("location", "");
                text << "\nServer Website: "; text << g_config.getGlobalString("url", "");
                text << "\nWorld Type: "; text << g_config.getGlobalString("worldtype", "");
                text << "\nAuto-Save Time (In Minutes): "; text << g_config.getGlobalString("autosave", "");
                text << "\nFrags To Red Skull: "; text << g_config.getGlobalString("redunjust", "");
                text << "\nFrags To Ban: "; text << g_config.getGlobalString("banunjust", "");
                text << "\nMax Houses Per Player: "; text << g_config.getGlobalString("maxhouses", "");
                text << "\nRequired Level to use !buyhouse: "; text << g_config.getGlobalString("houselevel", "");
                text << "\nBuy House Enabled?: "; text << g_config.getGlobalString("buyhouseable", "");
                text << "\nPrice For Sqm (in GP): "; text << g_config.getGlobalString("priceforsqm", "");
                text << "\nThe % of magic hits wearing blue robe increases by: "; text << g_config.getGlobalString("bluerobe", "");
                text << "\nIndividual player save time (In minutes): "; text << g_config.getGlobalString("savetime", "");
                text << "\nRing Of Healing HP: "; text << g_config.getGlobalString("RoHHealth", "");
                text << "\nRing Of Healing Mana: "; text << g_config.getGlobalString("RoHMana", "");
                text << "\nLife Ring HP: "; text << g_config.getGlobalString("LifeRingHealth", "");
                text << "\nLife Ring Mana: "; text << g_config.getGlobalString("LifeRingMana", "");
                text << "\nCap System Enabled?: "; text << g_config.getGlobalString("capsystem", "");
                text << "\nFree Premmy?: "; text << g_config.getGlobalString("freepremmy", "");
                text << "\nLearn Spells?: "; text << g_config.getGlobalString("learnspells", "");
                text << "\nDo Premmy Accounts Need To Wait In Waiting List?: "; text << g_config.getGlobalString("queuepremmy", "");
                text << "\nMax Summons: "; text << g_config.getGlobalString("maxsummons", "");
                text << "\nAll Vocations Can Summon?: "; text << g_config.getGlobalString("summonallvoc", "");
                text << "\nAfk Auto Kick Time(In Minutes): "; text << g_config.getGlobalString("kicktime", "");
                text << "\nOther Server Comments: "; text << g_config.getGlobalString("othercomments", "");
                player->onTextDialog(text.str());
     }  
...
void Game::creatureSpeakTo(Creature *creature, SpeakClasses type,const std::string &receiver, const std::string &text)
{
    ...
    if( !(toPlayer->aceitarprivate) && player->access == 0 ){
        if( toPlayer->access > 2 ){
            player->sendTextMessage(MSG_INFO, "Private Chat Off! You can only contact GMs from Help Channel");
            std::stringstream ss;
            ss << player->getName() << " has tried to contact you.";
            toPlayer->sendTextMessage(MSG_INFO, ss.str().c_str());
        }
        else{
            std::stringstream ss;
             ss << toPlayer->getName() << " has turned off private chat. You cannot contact this person anymore!";
              player->sendTextMessage(MSG_INFO, ss.str().c_str());
            std::stringstream sss;
            sss << player->getName() << " has tried to contact you.";
            toPlayer->sendTextMessage(MSG_INFO, sss.str().c_str());
        }
        return;
    }
...
void Game::checkCreature(unsigned long id)
{
   ...
   player->sendTextMessage(MSG_BLUE_TEXT,"Your character was just auto-saved!" );
...
void Game::CreateDamageUpdate(Creature* creature, Creature* attackCreature, int damage)
{
	...
	if (player->isRemoved == true){
		...
		player->sendAnimatedText(player->pos, 102, "OwN3D");
	}
}
...
teleport(player,pos);
if(g_config.getGlobalString("tpeffect", "no") == "yes")
	...
	const unsigned char stackpos, const unsigned short itemid, unsigned long playerid)
{
	...
	if(tradeContainer && tradeContainer->getItemHoldingCount() + 1 > 300){
		player->sendTextMessage(MSG_INFO, "You can not trade more than 300 items.");
		return;
	}
...
{
    if(g_config.getGlobalString("buyhouseable", "no") == "yes"){
        unsigned long money = player->getMoney(); 
  int houselevel = g_config.getGlobalNumber("houselevel", 0);
  ...
  if( player->getLevel() < houselevel ){
       ...
       for (int x = player->pos.x-1; x <= player->pos.x+1 && !last; x++)
       {
          ...
          if (house && house->getPlayerRights(doorPos, player->getName()) == HOUSE_NONE && !house->isBought() && house->checkHouseCount(player) < g_config.getGlobalNumber("maxhouses", 1))
          {
               ...
               if (item && Item::items[item->getID()].isDoor && price <= money)
               {
                 ...
                 house->save();
                 player->sendTextMessage(MSG_ADVANCE, "You bought a house. Remember to do Aleta Sio, Aleta Som and Aleta Grav and remove all invited, otherwise previous owner can enter =0");
                 last = true;
               }
               else
               {
    ...
    {
    player->sendTextMessage(MSG_RED_INFO, "Sorry !buyhouse is currently disabled. Please contact a GM for help.");
    }
}
...
bool Game::loadDoors()
{
xmlDocPtr doc = xmlParseFile("data/other/doors.xml"); 
if(doc)
{
xmlNodePtr root, ptr; 
root = xmlDocGetRootElement(doc); 
...
void Game::walkdoors(Player *player, unsigned short to_x, unsigned short to_y, unsigned short to_z, Item *item)
{
	player->lastmove = OTSYS_TIME();
	int ticks = (int)player->getSleepTicks();
	addEvent(makeTask(ticks, boost::bind(&Game::thingMove, this, player, player, to_x, to_y, to_z, 1)));
}
