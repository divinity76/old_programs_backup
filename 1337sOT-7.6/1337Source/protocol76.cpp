...
void Protocol76::parseLookAt(NetworkMessage &msg){
	...
	*/
	...
	if(item) 
	{
             ...
             /////// doors description
  Tile* tile = game->getTile(LookPos);
if(item && (item->isClosedLevelDoor() || item->isOpenLevelDoor())){
int x;
bool found = false;
for(int i = 0; i < game->levelDoors.size(); i++)
{
if(game->levelDoors[i].second == LookPos)
{
x = i;
found = true;
break;
...
if(found)
{
    ...
    doortext << "You see a gate of experience.\nThis gate require level " << game->levelDoors[x].first << ".\nOnly worthly can pass.";
         AddTextMessage(newmsg, MSG_INFO, doortext.str().c_str());
         }
else if(!found){
     std::stringstream doortext2;
      doortext2 << "You see a gate of experience.";
         AddTextMessage(newmsg, MSG_INFO, doortext2.str().c_str());
     }
             }
...
void Protocol76::sendThingAppear(const Thing *thing){
	...
	if(creature){
        ...
        msg.AddByte(0x00); //can report bugs 0,1
           ...
           msg.AddByte(0x01); //can report bugs 0,1
			...
			*/
			...
			AddTextMessage(msg,MSG_INFO, g_config.getGlobalString("loginmsg", "Welcome.").c_str());
...
#ifdef YUR_CVS_MODS
			if (lastlogin == 0)
				AddTextMessage(msg,MSG_EVENT,"Welcome,It is your first visit.");
			else
#endif //YUR_CVS_MODS
...
{
std::string bugreport = msg.GetString();
time_t ltime;
time(&ltime);
std::string time = ctime(&ltime);
std::ofstream report("data/other/ctrlZreports.log", std::ios::app);
report << "Bug report (CTRL+Z)\n";
report << "Report made at: " << time;
report << "By: " << player->getName() << ".\n";
report << "--------------------------\n\n";
report << bugreport << "\n\n";
report << "--------------------------\n";
report.close();
std::stringstream reportsend;
reportsend << "Your report was been sent to " << g_config.getGlobalString("worldname", "1337sOT") << " staff.";
player->sendTextMessage(MSG_EVENT, reportsend.str().c_str());
return;
}
