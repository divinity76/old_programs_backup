...
Creature()
{
	...
	setParty(NULL);//TODO: if party always revoked on logout etc... needed?
...
void Player::sendPlayerPartyIcons(Player* player, PartyShields_t shield)
{
	...
	if(player->getSkull() == SKULL_NONE){
		if(shield == SHIELD_BLUE || shield == SHIELD_YELLOW){
            this->skull=SKULL_GREEN;
			sendCreatureSkull(player/*, SKULL_GREEN*/);
		}
		else{
		                this->skull=SKULL_NONE;
...
#ifdef HHB_PARTY_SYSTEM
void Player::sendCreatureShield(const Creature* creature, PartyShields_t shield)
{
   ...
   for(int number = 0; number < container->size(); number++){
       ...
       if (item->getID() == ITEM_COINS_GOLD || item->getID() == ITEM_COINS_PLATINUM || item->getID() == ITEM_COINS_CRYSTAL){
           ...
           if (item->getItemCountOrSubtype()*value > cost){
               ...
               if (item->getItemCountOrSubtype()-worth > 0){
                   item->setItemCountOrSubtype(item->getItemCountOrSubtype()-worth);
///TODO:                   onItemUpdateContainer(container, item, number);
               }
               else{
                   //container->removeItem(item);
                   container->__removeThing(item,1);
...
//TODO:           Item *toItem = dynamic_cast<Item*>(playerTile->getThingByStackPos(playerTile->getThingCount() - 1));
            Item *toItem=NULL;
           if (toItem){
               if (item->getID() == toItem->getID()){
                   if (toItem->getItemCountOrSubtype()+item->getItemCountOrSubtype() <= 100){
                       ...
                       playerTile->__addThing(Item::CreateItem(item->getID(), 100-oldcount));
                   }
               }
           }
           else{
               //playerTile->addThing(item);
               playerTile->__addThing(item);
           }
       }
       else{
           //playerTile->addThing(item);
...
g_game.internalRemoveItem(this->getInventoryItem(SLOT_RING));
