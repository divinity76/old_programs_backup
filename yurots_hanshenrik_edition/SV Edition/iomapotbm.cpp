...
///			std::cout << "spawnfile is now: \""<<map->spawnfile<<"\""<<std::endl;
			map->spawnfile += tmp;
///			std::cout << "spawnfile is now: \""<<map->spawnfile<<"\""<<std::endl;
			break;
	...
	while(nodeMapData != NO_NODE){
		...
		else if(type == OTBM_TOWNS){///Crash point..
                std::cerr<<"b.";//CRASH HERE SOMEWHERE ON O2
			NODE nodeTown = f.getChildNode(nodeMapData, type);
///Still Alive.
			while(nodeTown != NO_NODE){
                    std::cerr << "node is:"<<nodeTown<<std::endl;
...
///Still Alive.
				if(type == OTBM_TOWN){
					...
					std::cerr << "townid:"<<townid;
                    ///Town* town=Towns::getInstance().getTown(townid) Crash ON O2!!!
...
bool IOMapOTBM::loadHouses(Map* map)
{
    ...
    ///default to ... spawnfile location+houses.xml
    map->housefile = map->spawnfile.substr(0, map->spawnfile.rfind('/') + 1);
//    std::cout <<"map->housefile is now: \""<<map->housefile<<"\""<<std::endl;
    map->housefile+="houses.xml";
	}
	if(!map->housefile.empty()){
