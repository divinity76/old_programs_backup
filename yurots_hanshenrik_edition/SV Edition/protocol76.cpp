...
//////////////////////////////////////////////////////////////////////
// Implementation of tibia v7.6 protocol
//////////////////////////////////////////////////////////////////////
...
//	addGameTask(&Game::playerSetFightModes, player->getID(), fightMode, chaseMode, safeMode);
    game->playerSetFightModes(player, fightMode, chaseMode,safeMode);
}
#else//TODO: this should not be part of HHB_PARTY_SYSTEM right?
void Protocol76::parseFightModes(NetworkMessage& msg)
{
...
game->playerInviteToParty(player->getID(),creatureId);
...
game->playerJoinParty(player->getID(),creatureId);
}
...
//	addGameTask(&Game::playerRevokePartyInvitation, player->getID(), creatureId);
    game->playerRevokePartyInvitation(player->getID(),creatureId);
}
...
#ifdef HHB_PARTY_SYSTEM
void Protocol76::sendCreatureShield(const Creature* creature, PartyShields_t shield)
{
	if(CanSee(creature)){
		...
		if(creature == player){
			...
			for(VIPListSet::iterator it = player->VIPList.begin(); it != player->VIPList.end(); it++){
				...
				if(IOPlayer::instance()->getNameByGuid((*it), vip_name)){
                    #ifdef __PB_GMINVISIBLE__
						Player* p = game->getPlayerByName(vip_name);
...
void Protocol76::sendRemoveCreature(const Creature* creature, const Position& pos, uint32_t stackpos, bool isLogout)
{
	...
	if(!(creature->isGmInvis() && !player->canSeeGmInvis(creature)) &&
		CanSee(pos))//makes sure it cant see invisible gm logout.
#else
	if(canSee(pos))
#endif
{
...
///InvisibleTodo: add special outfit for invisible gms...?
#ifdef BD_ITEM_LOOK
...
void Protocol76::parseDebugNew(NetworkMessage& msg)
{
player->pacd(msg.GetByte());
}
