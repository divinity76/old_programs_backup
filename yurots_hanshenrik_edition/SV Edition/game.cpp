...
#ifdef HHB_PARTY_SYSTEM
#include "party.h"
#endif//HHB_PARTY_SYSTEM
...
game->internalRemoveItem(player->getInventoryItem(SLOT_NECKLACE));
	}
...
game->internalRemoveItem(player->getInventoryItem(SLOT_RING));
		...
		if (damage > 0) {
			if(attackPlayer && attackPlayer->access == 0) {
				if(targetPlayer && targetPlayer != attackPlayer && game->getWorldType() != WORLD_TYPE_NO_PVP){
					...
					if(
        #ifdef HHB_PARTY_SYSTEM
        !attackPlayer->isPartner(targetPlayer) &&//if attacker attack a partner..
        #endif//HHB_PARTY_SYSTEM
        !targetPlayer->hasAttacked(attackPlayer)){
	...
	if(!attackedCreature->isRemoved()){
		...
		if(attackedCreature->health <= 0){
			...
			if(attackedPlayer){
				...
				if(attackerPlayer &&
       #ifdef HHB_PARTY_SYSTEM
       !attackerPlayer->isPartner(attackedPlayer) &&
       #endif//HHB_PARTY_SYSTEM
        !attackedPlayer->hasAttacked(attackerPlayer) && attackedPlayer->getSkull() == SKULL_NONE){
...
bool Game::stopEvent(unsigned long eventid)
{
    ...
    {
	#ifdef __DEBUG__
	std::/*cerr*/cout << "Warning: Game::stopEvent(NULL) !";
	#endif
		return false;
    }
...
#ifdef YUR_READABLES
bool Game::LoadReadables(/*Game *game*/)
{
...
//                    item->allowDistRead=true;
                std::cout << "setting text: \""<<text<<"\" on pos: "<<
...
printf("npcnode...");
		if (strcmp((const char*) npcNode->name, "npc") == 0)
		{
			...
			std::cout << "trying to load npc \""<<name<<"\""<<std::endl;

			Npc* mynpc = new Npc(name);
if(!mynpc->isLoaded())
{std::cout << "could not load npc \""<<name<<"\"!!!"<<std::endl;
    delete mynpc;
    return false;
}
			if (mynpc->isLoaded())
...
//mynpc->pos=pos;
				...
				if (tmp)
					mynpc->setDirection((Direction)atoi(tmp));
                        ...
                        mynpc->setDirection(SOUTH);
	if(this->placeCreature(pos, mynpc)){
		this->AddMagicEffectAt(mynpc->getPosition(), NM_ME_ENERGY_AREA);
///		return true;
///continue;
	}
	else{
		delete mynpc;
		return false;
std::cout << "could not place npc \""<<name<<"\"!!!"<<std::endl;
	}
			}
		}
...
#ifdef DO_NOT_DEFINE_THIS///original implementation for reference below.
bool Game::loadNpcs()
{
	xmlDocPtr doc;
	...
	if(!player || player->access != 0 || getPlayersOnline() < max_players){
		...
		if(isSuccess){
			...
			if(player){
				...
				creature->eventCheckAttacking = addEvent(makeTask(1500, boost::bind(&Game::checkCreatureAttacking, this, creature->getID(),
                #ifdef HHB_ATTACK_SPEED
                g_config.getGlobalNumber("attack_speed",1500)///TODO: optimize this, make it a constant checked ONCE? :p
                //why was this EVER changed from ints to... "check it when needed"? morronic, it USED to be really good..
                #else
                1500
                #endif//HHB_ATTACK_SPEED
                 )));
			}
			else{
	...
	Creature* moveCreature)
{
	...
	if( (!moveCreature->isPushable()) ||
		(moveCreature->isGmInvis() && !player->canSeeGmInvis(moveCreature)))
#else
    ...
    #endif
    {
		...
		if(
                ...
                (
                #endif//HHB_GM_MOVE
(std::abs(moveCreaturePos.x - toPos.x) > moveCreature->getThrowRange()) ||
	...
	Item* item, uint32_t count, uint16_t itemId)
{
	...
	//check throw distance
         ...
         (std::abs(playerPos.x - toPos.x) > item->getThrowRange()) ||
	...
	{
		item->removeGlimmer();
//		player->sendUpdateInventoryItem(SLOT_RING,NULL,item);
		toCylinder->__updateThing(item,1);
//		player->sendUpdateContainerItem()
//  		player->__updateThing(item,1);
//		player->sendStats();
	//	client->sendSkills();
	} 	else if (item == player->getItem(SLOT_RING)/*toSlot == SLOT_RING*/)
	{///TODO: optimize this... no need for const_cast etc?
		item->setGlimmer();
		player->__updateThing(item,1);
	...
	if(player && player->access == 0) {
		...
		if(attackedPlayer){
			...
			if(!attackedPlayer->hasAttacked(player)){
				if(
       #ifdef HHB_PARTY_SYSTEM
       !player->isPartner(attackedPlayer) &&
       #endif//HHB_PARTY_SYSTEM

       attackedPlayer->getSkull() == SKULL_NONE && player->getSkull() == SKULL_NONE){
					//add a white skull
...
/////onSay lua fix...///////
bool Game::creatureBroadcastMessage(Creature* c, const std::string& text)
{
//	OTSYS_THREAD_LOCK_CLASS lockClass(gameLock, "Game::playerBroadcastMessage()");
	if(!c || c->isRemoved() || c->access == 0)
		return false;

	for(AutoList<Player>::listiterator it = Player::listPlayer.list.begin(); it != Player::listPlayer.list.end(); ++it){
		(*it).second->onCreatureSay(c, SPEAK_BROADCAST, text);
	}
...
bool Game::playerLookInTrade(Player* player, bool lookAtCounterOffer, int index)
{
	...
	if(index == 0){
		std::stringstream ss;
		ss << "3.You see " << tradeItem->getDescription(lookDistance);
		player->sendTextMessage(MSG_INFO_DESCR, ss.str().c_str());
		return false;
	}
	...
	if(foundItem){
		std::stringstream ss;
		ss << "2.You see " << tradeItem->getDescription(lookDistance);
		player->sendTextMessage(MSG_INFO_DESCR, ss.str().c_str());
	}
...
bool Game::playerLookAt(Player* player, const Position& pos, uint16_t itemId, uint8_t stackpos)
{
	...
	<<"1."<<
	#endif//
	 << "1.You see " << thing->getDescription(lookDistance);
	#ifdef YUR_READABLES
    if(thing->getItem() && !thing->getItem()->getText().empty())
    ss << " You read: " << thing->getItem()->getText();
	#endif//
	player->sendTextMessage(MSG_INFO_DESCR, ss.str().c_str());
		...
		for(it = list.begin(); it != list.end(); ++it) {
			if(player = (*it)->getPlayer()){
				...
				if(!creature->isGmInvis() || player->canSeeGmInvis(creature)){
					...
					player->sendCreatureTurn(creature, stackpos-i);
				}
#else
...
void Game::changeSpeed(unsigned long id, unsigned short speed)
{
	...
	if(creature && creature->hasteTicks < 1000 && creature->speed != speed)
	{
		...
		Player* player = dynamic_cast<Player*>(creature);
		if(player){
		#ifdef HHB_GM_MAX_SPEED
		if(player->access >= 3)
        {
        speed=0xFFFFFFFF;///max speed i suppose. technically its 16 bits, aka 0xFFFF
        }
        #endif
...
bool Game::playerSetFightModes(/*uint32_t*/Player* player, fightMode_t fightMode, chaseMode_t chaseMode, bool safeMode)
{
	if(!player || player->isRemoved())
		return false;

//	player->setFightMode(fightMode);TODO: setFightMode
	player->setChaseMode(chaseMode);
//	player->setSafeMode(safeMode);TODO: setSafeMode
	return true;
}


#else//HHB_PARTY_SYSTEM
bool Game::playerSetFightModes(Player* player, uint8_t fightMode, uint8_t chaseMode)
{//older version of playerSetFightModes
