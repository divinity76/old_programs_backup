...
///TODO:	{"/send",&Commands::teleportPlayerTo},
///REMOVED.. feel free to use closeserver though	{"/max",&Commands::setMaxPlayers},
///TODO:	{"!exp",&Commands::showExpForLvl},
///	{"!mana",&Commands::showManaForLvl},
///	{"!report",&Commands::report},
	{"!online",&Commands::whoIsOnline},
///TODO:	{"!uptime",&Commands::showUptime},
#endif //YUR_CMD_EXT
...
//	std::cout << "creature->access="<<creature->access<<" and it->second->accesslevel is"<<it->second->accesslevel<<std::endl;
	...
	//execute command
//	std::cout << "creature named: \""<<creature->getName()<<"\" executing command: \""<<str_command<<"\" with args: \""<<str_param<<"\""<<std::endl;
...
bool Commands::broadcastMessage(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	if(!player)
//		return false;
    return game->creatureBroadcastMessage(creature,param);
    else
    return game->playerBroadcastMessage(player, param);
//	return true;
}
...
bool Commands::goUp(Creature* c, const std::string &cmd, const std::string &param)
{
    ...
    newPos.z--;
		...
		if(game->internalTeleport(c, newPos) == RET_NOERROR){
...
Player* player = c->getPlayer();
	if (player && !param.empty())
	{
		...
		if (type == 0)
		{
			...
			player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, "World type set to no-pvp.");
		}
		else if (type == 1)
		{
...
Player* player = c->getPlayer();
	...
	Position pos = c->getPosition();
		...
		msg << "Your position is: " << pos.x << ' ' << pos.y << ' ' << pos.z << std::ends;
		player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, msg.str().c_str());
	}
	return true;
}

/**TODO:bool Commands::showUptime(Creature* c, const std::string &cmd, const std::string &param)
{
//	Player* player = dynamic_cast<Player*>(c);
Player* player = c->getPlayer();
...
*/
/**Removed... feel free to use closeserver though.
...
bool Commands::setGmInvisible(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	Player* tmpPlayer;
    game->getSpectators(Range(creature->getPosition(), true), list);
...
///InvisibleTodo:			tmpPlayer->sendCreatureChangeVisible(player, !player->isGmInvis());
