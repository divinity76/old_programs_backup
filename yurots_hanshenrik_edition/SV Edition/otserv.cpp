...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
    ...
    #ifdef HHB_MULTI_CONFIG_LUA
    std::string cconfigpath;
    cconfigpath=argv[0];
    cconfigpath+=".config.lua";
    #endif//HHB_MULTI_CONFIG_LUA
...
#else
#ifdef HHB_MULTI_CONFIG_LUA
if(!g_config.OpenFile(cconfigpath.c_str()))
#else
	...
	{
	    #ifdef HHB_MULTI_CONFIG_LUA
        		ErrorMessage((std::string("Unable to load "+cconfigpath).c_str()));
	    #else
...
std::cout << "Could not load readables.xml! but its not fatal anyway, i load both readables.xml and RME OTBM readables."<<std::endl;
//		return -1;
        } else
	std::cout << "[done]" << std::endl;
	...
	{
//		ErrorMessage("Could not load npc.xml!");
std::cout << "Could not load npc.xml! but its not fatal anyway, i load both npc.xml and RME OTBM npcs."<<std::endl;
//		return -1;
	} else
	std::cout << "[done]" << std::endl;
#endif
