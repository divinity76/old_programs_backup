...
{
    //std::cout << "trying to load houses filename: \""<<filename<<"\""<<std::endl;
	...
	if(doc){
		...
		while(houseNode){
			if(xmlStrcmp(houseNode->name,(const xmlChar*)"house") == 0){
                ...
                std::cout << "error loading houses ("<<filename<<"), i cannot find <houseid=\"NUMBER\"/>";
                //std::cout << "loadhouses error 2";
					return false;
				}

				House* house = Houses::getInstance().getHouse(_houseid);
				if(!house){
                  //std::cout << "loadhouses error 3";
