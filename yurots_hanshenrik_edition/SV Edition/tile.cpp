	...
	for(it = list.begin(); it != list.end(); ++it){
		if(player = (*it)->getPlayer()){
			...
			for(vit = v.begin(); vit != v.end(); ++vit){
				if((*vit)->isGmInvis() && !player->canSeeGmInvis((*vit))){
					i--;
				}
			}
///TODO:			player->sendRemoveTileItem(this, cylinderMapPos, i, item);
...
//				player->sendCreatureMove(creature, toTile, toPos, this, fromPos, oldStackPos-i, teleport);
				player->sendCreatureMove(creature, fromPos, oldStackPos-i, teleport);
