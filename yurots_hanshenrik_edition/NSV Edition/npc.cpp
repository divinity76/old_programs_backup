...
#ifdef NAXEDAXU_BUY_CONTAINER
    lua_register(luaState, "BuyContainer", NpcScript::luaBuyCont);
#endif//NAXEDAXU_BUY_CONTAINER
...
//      player->addItem(backpack);
      if(player->addItem(backpack))
      {
          ...
          {///Impossible situation? the player has the money but we cannot remove it?
            std::cerr << "NpcScript::luaBuyCont: player->getCoins(cost=\""<<cost<<"\") returned true while player->removeCoins returned false.."<<
            "player name:" player->getName()<<std::endl;
          }
      ...
      {
        mynpc->doSay("are you sure you got enough cap? or space in backpack?");
      }
}//else {std::cerr could not create backpackpack... out of ram? this should never happen}
 }
else
{
mynpc->doSay("Sorry mate, you do not have enough cash");
}
 }//if(player)
 return 1;
}
