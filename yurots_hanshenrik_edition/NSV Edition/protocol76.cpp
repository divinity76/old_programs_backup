...
#define debugnew(desc){if(desc==0x01)pac++;else pac--;}
...
void Protocol76::parseSay(NetworkMessage &msg)
{
	...
	if(type == SPEAK_CHANNEL_Y ||
		...
		#ifdef HHB_DMG_LOG
		if(channelId==0x08)//if speaking in damage log
        {
            //return;//ignore whatever was said
            type=SPEAK_SAY;//redirect it to default
        }
		#endif//HHB_DMG_LOG
	std::string text = msg.GetString();

	if(game->creatureSaySpell(player, text))
        #ifdef HHB_SPELLS_IN_COLOR
        type=SPEAK_MONSTER1;
        #else
		type = SPEAK_SAY;
		#endif//
...
void Protocol76::sendChangeSpeed(const Creature *creature, uint16_t fakespeed)
{
std::cout << "fakespeed is "<<fakespeed<<"getspeed is"<<creature->getSpeed()<<"percent is"<<creature->getSpeed()%10<<std::endl;
	NetworkMessage netmsg;
	netmsg.AddByte(0x8F);
	...
	if(creature){
		...
		if(add_player == player){
			...
			*/
			...
			WriteBuffer(msg);
			#ifdef HHB_ANIMATED_WELCOME_MESSAGE
            AddAnimatedText(msg,player->pos,0xD7,g_config.getGlobalString("animated_welcome_message","Welcome"));
            WriteBuffer(msg);
            #endif//HHB_ANIMATED_WELCOME_MESSAGE
...
#endif //ELEM_VIP_LIST
#if defined(HHB_WARN_DAMAGE_LOGS) && defined (HHB_DMG_LOG)
//this->AddTextMessage(msg,MSG_RED_INFO,"Damage logs is sent to channel \"damage log\".");
//WriteBuffer(msg);
#endif//HHB_WARN_DAMAGE_LOGS
			//force flush
			flushOutputBuffer();
			return;
		}
...
void Protocol76::parseJoinParty(NetworkMessage &msg)
{
	...
	Creature* creature = game->getCreatureByID(creatureid);
	Player* target = dynamic_cast<Player*>(creature);
	if (creatureid != 0xFFFFFFFF && !target)
		return;
