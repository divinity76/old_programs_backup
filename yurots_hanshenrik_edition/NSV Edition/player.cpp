...
#ifdef YUR_GUILD_SYSTEM//TODO: clean this mess up?
#ifdef YUR_GUILD_SYSTEM
...
void Player::addSkillTryInternal(int skilltry,int skill){
	...
	if (skills[skill][SKILL_TRIES] >= getReqSkillTries(skill, (skills[skill][SKILL_LEVEL] + 1), vocation)) {
	    if(vocation == VOCATION_KNIGHT && skills[skill][SKILL_LEVEL] != 15)
...
void Player::sendChangeSpeed(Creature* creature,uint16_t fakespeed){
     client->sendChangeSpeed(creature,fakespeed);
}
