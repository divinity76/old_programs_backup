...
public:
...
#ifdef HHB_GCC_4X///TODO: this was taken from original PropStream from 3x.. to make it compile..  not sure what it does
	template <typename TT>
	inline bool GET_STRUCT(TT* &ret){
		if(size() < sizeof(TT)){
			ret = NULL;
			return false;
		}
	...
	inline bool GET_UCHAR(unsigned char &ret){//GET_UINT8??...
		return GET_VALUE(ret);
	}
	inline bool GET_ULONG(unsigned long &ret){//GET_UINT64 ?? ...
		return GET_VALUE(ret);
	}


	inline bool GET_USHORT(unsigned short &ret){//GET_UINT16 ??...
		return GET_VALUE(ret);
	}
#endif//HHB_GCC_4X///TODO
...
///had to move this from protected to public... why? not investigated. needed to compile. TODO. (probably a friend class? dunno)
	template <typename T>
